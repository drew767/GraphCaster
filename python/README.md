...
**Данные предиката:** копия публичного контекста (**`last_result`**, **`node_outputs`**, …) плюс **`$json`** — снимок **`last_result`**: если это **`dict`**, используется он; иначе **`{"value": last_result}`** (эргономика как **n8n** **`$json`** для текущего item). Ключ **`$json`** в **`context`** извне при оценке условия **перезаписывается** снимком **`last_result`** (избежание подмены в выражениях).
