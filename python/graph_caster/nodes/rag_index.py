...
"""rag_index node: ingest text into the in-process vector store for this graph run."""
...
def execute_rag_index(
    *,
    ...
    if reason:
        return False, {
            "processResult": {"success": False, "exitCode": 1, "timedOut": False, "error": reason},
...
class RagIndexNode(GraphCasterNode):
    type: ClassVar[str] = "rag_index"
    version: ClassVar[float] = 1.0
    display_name: ClassVar[str] = "RAG Index"
    description: ClassVar[str] = "Index text chunks into the in-process vector store"
    category: ClassVar[str] = "ai"
    icon: ClassVar[str] = "database"

    inputs: ClassVar[list[Input]] = [
        Input("collectionId", str, required=True),
        Input("text", str, required=True, multiline=True),
        Input("chunkSize", int, default=512, range=(32, 8192)),
        Input("chunkOverlap", int, default=64, range=(0, 4096)),
        Input("embeddingDims", int, default=64, range=(8, 4096)),
        Input("mode", str, default="replace", options=["replace", "append"]),
    ]
    outputs: ClassVar[list[Output]] = [
        Output("ragIndexResult", "json"),
