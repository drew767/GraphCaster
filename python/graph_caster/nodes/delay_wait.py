...
"""delay / debounce / wait_for nodes: fixed sleep, debounce sleep, wait-for-file polling."""
...
def interruptible_sleep(
    ...
    *,
    ...
    """Sleep up to ``total_sec`` in small slices. Returns False if cancelled."""
    if total_sec <= 0:
        return True
...
class DelayNode(GraphCasterNode):
    type: ClassVar[str] = "delay"
    version: ClassVar[float] = 1.0
    display_name: ClassVar[str] = "Delay"
    description: ClassVar[str] = "Pause execution for a fixed duration"
    category: ClassVar[str] = "flow"
    icon: ClassVar[str] = "clock"

    inputs: ClassVar[list[Input]] = [
        Input("durationSec", float, required=True, range=(0.0, _MAX_DURATION_SEC)),
    ]
    outputs: ClassVar[list[Output]] = [
        Output("waitResult", "json"),
    ...
    async def run(self, ctx: Any, **kwargs: Any) -> dict[str, Any]:
        ...
        _ok, patch = execute_delay_or_debounce(
            node_id=node_id,
            graph_id=graph_id,
            wait_kind="delay",
            data=kwargs,
...
class DebounceNode(GraphCasterNode):
    type: ClassVar[str] = "debounce"
    version: ClassVar[float] = 1.0
    display_name: ClassVar[str] = "Debounce"
    description: ClassVar[str] = "Quiet-period wait (same sleep semantics as delay)"
    category: ClassVar[str] = "flow"
    icon: ClassVar[str] = "clock"
...
class WaitForNode(GraphCasterNode):
    type: ClassVar[str] = "wait_for"
    version: ClassVar[float] = 1.0
    display_name: ClassVar[str] = "Wait For"
    description: ClassVar[str] = "Poll until a workspace file exists or timeout"
    category: ClassVar[str] = "flow"
    icon: ClassVar[str] = "hourglass"

    inputs: ClassVar[list[Input]] = [
        Input("waitMode", str, default="file", options=["file"]),
        Input("path", str, required=True, description="Workspace-relative path"),
        Input("timeoutSec", float, default=300.0, range=(0.0, _MAX_DURATION_SEC)),
        Input("pollIntervalSec", float, default=0.25, range=(_MIN_POLL_SEC, _MAX_POLL_SEC)),
    ]
    outputs: ClassVar[list[Output]] = [
        Output("waitResult", "json"),
        Output("processResult", "json"),
    ...
    async def run(self, ctx: Any, **kwargs: Any) -> dict[str, Any]:
        ...
        _ok, patch = execute_wait_for_file(
            node_id=node_id,
            graph_id=graph_id,
            data=kwargs,
...
register_class(DelayNode)
register_class(DebounceNode)
register_class(WaitForNode)
