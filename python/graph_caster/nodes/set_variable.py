...
"""set_variable node: mutate ``ctx["run_variables"]`` via set / increment / append / delete."""
...
class SetVariableNode(GraphCasterNode):
    type: ClassVar[str] = "set_variable"
    version: ClassVar[float] = 1.0
    display_name: ClassVar[str] = "Set Variable"
    description: ClassVar[str] = "Update a run-scoped variable (set / increment / append / delete)"
    category: ClassVar[str] = "core"
    icon: ClassVar[str] = "variable"

    inputs: ClassVar[list[Input]] = [
        Input("name", str, required=True, description="Variable name (identifier)"),
        Input(
            "operation",
            str,
            required=True,
            options=["set", "increment", "append", "delete"],
            default="set",
        ),
        Input("value", "json", default=None, description="New value / delta / appended item"),
    ]
    outputs: ClassVar[list[Output]] = [
        Output("setVariableResult", "json"),
    ...
    async def run(self, ctx: Any, **kwargs: Any) -> dict[str, Any]:
        ...
        if not isinstance(run_ctx, dict):
            run_ctx = {}
        data = {k: v for k, v in kwargs.items() if v is not None or k == "value"}
        _ok, patch = execute_set_variable(
            node_id=node_id,
            graph_id=graph_id,
            data=data,
            ctx=run_ctx,
...
register_class(SetVariableNode)
