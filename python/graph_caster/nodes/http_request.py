...
"""http_request node: in-process HTTP client (stdlib only)."""
...
def execute_http_request(
    *,
    ...
    """Perform one HTTP round-trip. Returns ``(success, patch)``."""
...
class HttpRequestNode(GraphCasterNode):
    ...
    inputs: ClassVar[list[Input]] = [
        ...
        Input("bodyLiteral", bool, default=False),
        Input("auth", "json", default=None, secret=True),
        Input("timeoutSec", float, default=_DEFAULT_TIMEOUT, range=(0.5, 3600.0)),
    ...
    async def run(self, ctx: Any, **kwargs: Any) -> dict[str, Any]:
        ...
        _ok, patch = execute_http_request(
            node_id=node_id,
            graph_id=graph_id,
            data=kwargs,
            ctx=run_ctx,
...
register_class(HttpRequestNode)
