...
"""task / llm_agent process nodes: run a subprocess with streaming stdio, retry, and timeout."""
...
from graph_caster.exec.io_pump import EmitFn, _communicate_with_streaming
from graph_caster.exec.llm_agent_process import run_llm_agent_process
from graph_caster.exec.process_errors import _eval_success, _record_task_process_result
...
def _resolve_argv_and_optional_preset_cwd(
    ...
    """Return ``(argv, preset_cwd, error)``; ``error`` set when ``gcCursorAgent`` invalid/missing CLI."""
...
class TaskNode(GraphCasterNode):
    type: ClassVar[str] = "task"
    version: ClassVar[float] = 1.0
    display_name: ClassVar[str] = "Task"
    description: ClassVar[str] = "Run an external command in a subprocess with streaming stdio"
    category: ClassVar[str] = "core"
    icon: ClassVar[str] = "terminal"

    inputs: ClassVar[list[Input]] = [
        Input("command", str, default=None, description="Command line (string or argv list)"),
        Input("argv", "json", default=None, description="Argv list alternative to ``command``"),
        Input("cwd", str, default=""),
        Input("timeoutSec", float, default=None, range=(0.0, 86400.0)),
        Input("retryCount", int, default=0, range=(0, 100)),
        Input("retryBackoffSec", float, default=1.0, range=(0.0, 600.0)),
        Input(
            "successMode",
            str,
            default="exit_code",
            options=["exit_code", "stdout_nonempty", "artifact_exists"],
        ),
        Input("env", "json", default=None, secret=True),
        Input("envKeys", "json", default=None),
    ]
    ...
    async def run(self, ctx: Any, **kwargs: Any) -> dict[str, Any]:
        ...
        workspace_secrets = getattr(ctx, "workspace_secrets", None)
        ok = run_task_process(
            node_id=node_id,
            graph_id=graph_id,
            data=kwargs,
        ...
        outs = run_ctx.setdefault("node_outputs", {}).get(node_id, {})
        pr = outs.get("processResult") if isinstance(outs, dict) else None
        return {"processResult": pr if isinstance(pr, dict) else {"success": ok}}


register_class(TaskNode)
