...
| **G. Наблюдаемость** | Логи исполнения, история, трейсинг, экспорт | Консоль UI + NDJSON (**§3.7**), артефакты **`runs/`**; стрим, жизненный цикл **`runId`**, **UX консоли** (фильтры, поиск, экспорт, переход к ноде — фаза 7); **offline-список прошлых прогонов + replay** (файловый журнал) — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md); **§17** — сравнение с конкурентами и **остаточный** **F13** (OTel, ORM-уровень **`Execution`** как у Flowise/n8n, транспорт **§39** / буфер **§39.2**), без дубля фактов UI |
