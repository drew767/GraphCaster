...
| **GraphCaster** | **частично** | Слой **B** (**полотно, инспектор, предупреждения**): закрытые срезы и факты по **F1** / производительности canvas — только [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md), раздел **«Canvas: большие графы»** и смежные пункты (**MiniMap**, поиск, ПКМ, мультивыбор, **F13**, **`group`** и т.д.). **Остаток F1 / §28.2:** опционально более лёгкий хром **полностью** вне вьюпорта (**§17**, профиль); полная типизация пинов — **§15**; **встраивание** дочернего графа в JSON родителя (**A**) вне file-first — **§29**. |
...
| **GraphCaster** | **@xyflow/react** | Редактор слоя **B** и сценарии **P1/P2** — единый реестр в [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (**«Canvas: большие графы»** в т.ч. **visible-only** + **LOD по zoom**, **MiniMap…**, **Поиск…**, меню добавления ноды, мультивыбор, **F13**, **`group`**, консоль, открытие графа и др.). **§28.2 п.4:** детали DOM/производительности не дублировать здесь; опционально — хром полностью вне кадра (**§17**); **§15**; встраивание child JSON в **A** — **§29** | **`GraphDocument`** + **`validate.py`**; без БД (**F23**) |
...
4. **Мини-карта / навигация / производительность полотна (п.4):** **Закрыто** — единственный перечень фактов реализации: [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (**«MiniMap и панель управления полотном»**, **«Поиск и переход к ноде на canvas»**, **«Canvas: большие графы»** — в т.ч. **`onlyRenderVisibleElements`**, **LOD по zoom** с гистерезисом, батчинг оверлея, sync рёбер, **ленивое превью `graph_ref`**; **F13** — там же). В **§28** детали **не** повторять. **Опционально по профилю (**§17**):** ещё более лёгкий хром для нод **полностью** вне кадра. **Встраивание** полного дочернего графа в документ родителя (**A**) — вне scope — **§29** (file-first).
