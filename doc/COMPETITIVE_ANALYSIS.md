...
| **G. Наблюдаемость** | Логи исполнения, история, трейсинг, экспорт | Консоль UI + NDJSON (**§3.7**), артефакты **`runs/`**; стрим, жизненный цикл **`runId`**, **UX консоли** (фильтры, поиск, экспорт, переход к ноде — фаза 7) — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md); **§17** — сравнение с конкурентами и **остаточный** **F13** (OTel, БД **`Execution`**, транспорт **§39** / буфер **§39.2**), без дубля фактов UI-консоли |
...
**Реализованный слой консоли** (фильтры, поиск, экспорт видимого/полного, sticky tail, переход к ноде) описан в [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md); ниже — **сравнение с конкурентами** и темы **F13**, которые в **`graph-caster`** пока **не** сведены к этому файлу (персистентность execution, OTel, транспорт **§39**).
...
| **GraphCaster** | Консоль по событиям (**§3.7**); детали UI — только [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (**F13**, фаза 7) | Каталог **`runs/`**, метаданные в **`artifacts`** | **нет** | **`runner.py`**, **`schemas/run-event.schema.json`**, **`ui/src/run/consoleLineMeta.ts`**, **`ConsolePanel`** |
