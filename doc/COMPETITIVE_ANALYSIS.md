...
**Открытие документа (P1, ошибки файла / JSON / парсера):** **закрыто** — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md), раздел **«Открытие графа: ошибки JSON и парсера»** (`OpenGraphErrorModal`, `openGraphErrorPresentation.ts`, **`titleWithFile`**); те же модалки для ошибок JSON в **инспекторе** (data ноды, inputs/outputs графа) и в **Save** (пустое имя, сбой записи) — в той же таблице раздела; детали и пути в коде — только там, **§28** не дублирует.
