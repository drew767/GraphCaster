...
| **Vibe Workflow** | — | — | нет |
| **GraphCaster** | **частично:** **`task`** + диск + **`gcPin`** + **десктоп:** тоггл **Step cache**, очередь **dirty** (транзитивно по успешным рёбрам → **`--step-cache-dirty`**, как **`dirtyNodeNames`** у n8n) — см. [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md); **полный** F17 — **§22.2** | Ключ: ревизия JSON + отпечаток предков + **`data`** ноды (см. [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md)) | **partial**; **`gcPin`** + межпрогонный кэш **`task`** под **`artifacts_base`** — **да** (CLI и Tauri); кэш не-**`task`** / TTL / транзитив **dirty** внутри вложенного **`graph_ref`** без модели id корня — **нет** (**§22.2**) |
