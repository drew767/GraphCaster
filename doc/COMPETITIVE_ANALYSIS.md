...
| **D. Рантайм** | Обход графа, условия на рёбрах, вложенные графы, субпроцессы, политика сбоев, опционально кэш шагов | `runner.py` (**§31** порядок обхода и достижимость, **F3**; **§32** условия на рёбрах и «первое истинное», **F4**; **§29** **`graph_ref`**, **F5**; диспетчер по **`kind`**, `unknown_node` — **§18**), **`process_exec.py`** (**§27**, **F7**), события NDJSON; внутренний слой «исполнитель → транспорт» (**`RunEventSink`**, **`StepQueue`**) — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md); **частичный** отладочный прогон (**`--until-node`**, **`--context-json`**) — там же («Частичный прогон»); полный **F6** (масштабирование прогонов, два уровня очередей как у **§13.3**) — **§13**; ошибки — **§16** / **§37** (**F19**); полный **F17** (кэш между прогонами / dirty-nodes) — **§22** / **§36** |
...
| **GraphCaster** | **частично** | Слой **D**: обход **`start`**→**`exit`**, **F4** (**§32**), вложение **`graph_ref`** (**§29**) — факты в [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md); **статическое предупреждение недостижимых из `start` нод** (over-approx., UI + **`validate.py`**) — там же, раздел **F3** / достижимость. Нода **`merge`** (**passthrough**, реконвергенция пути) — там же (**F4** / Merge). **§31.2:** **barrier**-join / параллельный fork как у полного **n8n Merge**, циклы, полная связность с учётом **F4** в рантайме — ещё открыто. |
...
| **GraphCaster** | **частично** | **Реализовано:** останов после выбранной ноды (**`--until-node`**, **`run_finished.status` `partial`**) и ручной merge **`node_outputs`** (**`--context-json`**), десктоп — см. [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (раздел «Частичный прогон»). **Нет** межпрогонного кэша выходов по **dirty**-узлам и автоматического **`pinData`** как у n8n/Comfy — **§22.2** + **§36.2** (ключ кэша, **`runs/`**, ревизия документа). |
...
2. **Не брать в MVP** без явного решения: **полный** **F6** (очередь **многих** прогонов, Redis/n8n queue mode, worker pool, in-graph fork/join) — **§13**; **внутренний** синхронный срез (sink + **StepQueue**) — уже в [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md). **F10** (ingestion + vector DB + раннер) — **§14**; **F14** (tenant, RBAC, SSO) — **§20** / **§38**, не дублировать Dify/n8n в `graph-caster`. **F8** — **§11** / **§35**. **F9**/**F12** — **§12**; триггеры — **§24**; публичный REST, ключи, OpenAPI — **§25**. **F22** (реалтайм co-edit, Yjs) — **§19**; не смешивать с **фазой 5** run-lock (локальная блокировка UX).
...
**Срез в GC (не полный F17):** отладочный partial (**`--until-node`**, **`run_finished.status` `partial`**) и ручной pin контекста (**`--context-json`**, **`node_outputs`**) — факты реализации в [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (раздел «Частичный прогон»). Ниже — сравнение конкурентов и **план** полного инкрементального кэша.
...
| **Vibe Workflow** | — | — | нет |
| **GraphCaster** | **частично:** нода (early stop / partial); **план:** полный F17 | **Уже:** **`--until-node`**, **`--context-json`** — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md). **План:** хэш входов + ревизия документа (**§22.2**) | **partial** — в одном процессе; межпрогонный кэш — **план** (**§22.2**) |
...
3. **Связь с артефактами (**E**): ручной pin через **`--context-json`** частично покрывает сценарий «взять прошлые выходы»; полноценный read-only pin из **прошлого** root-run (аналог **`pinData` n8n**) с меткой в UI «зафиксировано / живое» — **план**.
...
| **GraphCaster** | Ожидается **один входной** **`start`** и путь к **`exit`**; **порядок** исходящих рёбер из **A** задаёт перебор при ветвлении (**F4**). **Статический** список нод вне обхода от **`start`** (все рёбра считаются проходимыми) — см. [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (**F3**). Явная нода **`merge`** для визуальной реконвергенции после **F4** (**passthrough**) — там же, подраздел **Merge** в **F4**. | Вложение: синхронный **`graph_ref`** (**§29**) | Данные по связям документа; события **`node_execute`** и др. (**§3.7**) |
...
4. **Fork/join (несколько предков):** **MVP:** в **A** есть нода **`merge`** с семантикой **passthrough** при одном активном пути — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (**F4** / Merge); это **не** полный **n8n Merge** (барьер на N входах, параллельные ветки — **§13** / **F6**). До отдельного ТЗ не расширять семантику неявно (**PRODUCT_DESIGNE.md**).
...
**Точечные углубления позже:** сравнение **Comfy** `get_input_info`/ordering с порядком рёбер GC; политика **n8n** «нода ждёт все входы merge» — при проектировании **barrier**-режима для **`merge`** в GC ([`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md)).
...
2. **Fork/join (полный **n8n**):** несколько **параллельных** живых веток и **barrier**-**merge** («дождаться всех входов») — вне текущей модели (**§31.2**, **§13**); нода **`merge`** сейчас только **passthrough** — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md); условные рёбра сами по себе **n8n**-merge не реализуют.
