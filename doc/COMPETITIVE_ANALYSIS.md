...
| **G. Наблюдаемость** | Логи исполнения, история, трейсинг, экспорт | Консоль UI + NDJSON (**§3.7**), артефакты **`runs/`**; стрим, жизненный цикл **`runId`**, **UX консоли** (фильтры, поиск, экспорт, переход к ноде — фаза 7); **оверлей статусов нод на канвасе** (live и replay из журнала — **`nodeRunOverlay`**, **`GraphCanvas`**) — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (**F13**); **offline-список прошлых прогонов + replay** (файловый журнал; проверка **`rootGraphId`** при replay — там же); **bounded queue** dev run-брокера (**SSE** / **WebSocket**, **`stream_backpressure`**) — там же (**«Backpressure SSE»** + **Evidence**); **нормализация финала при обрыве воркера** (synthetic **`run_finished`**) — там же, строка **«Обрыв воркера»** в **«Десктоп…»**; **§17** — сравнение с конкурентами и **остаточный** **F13** (OTel, ORM-уровень **`Execution`** как у Flowise/n8n, **prod**-транспорт **§39** / буфер **§39.2** без dev-брокера), без дубля фактов UI |
...
| **GraphCaster** | **частично** | Слой **B**: **@xyflow/react**, **`GcFlowNode`** / **`GcCommentNode`**, инспектор, предупреждения — **закрытые** срезы (**F18**, undo/redo, run-lock, sanitize, мини-карта, поиск, ПКМ-меню, мультивыбор, **производительность больших графов** волна 1, F13 оверлей, **CI**) — единый реестр в [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (**«MiniMap…»**, **«Поиск…»**, **«Меню «Добавить ноду»…»**, **«Мультивыбор…»**, **«Canvas: большие графы»** и смежные разделы). **Остаток F1 / §28.2:** **lazy**-подграфы в схеме **A**, группы (frame) в документе (**§28.2** п.7), опционально «лёгкий» хром ноды вне вьюпорта после профиля; полная типизация пинов — **§15**. |
...
| **GraphCaster** | Консоль по событиям (**§3.7**), в т.ч. инкрементальный вывод **`task`** (**`process_output`** в NDJSON); **оверлей статусов нод на канвасе** (live/replay, **`nodeRunOverlay.ts`**, **`GraphCanvas`**, i18n **`app.run.overlay.*`**) — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (**F13**) | **File-first:** **`events.ndjson`** + **`run-summary.json`** при **`--artifacts-base`**; список/чтение через Tauri и **`POST /persisted-runs/*`**; UI **History** + replay с тем же оверлеем; предупреждение при несовпадении **`rootGraphId`** лога и открытого графа — там же | **нет** | **`run_event_sink.py`**, **`artifacts.py`**, **`runner.py`**, **`process_exec.py`**, **`run_broker/`**, **`run_bridge.rs`**, **`RunHistoryModal`**, **`schemas/run-event.schema.json`**, **`ConsolePanel`**, **`parseRunEventLine.ts`**, **`runEventSideEffects.ts`**, **`runSessionStore.ts`** |

### 17.2. Планирование для GC

1. **Не смешивать фазы:** фаза **7** — фильтры, поиск, экспорт из консоли, привязка к ноде (**сделано** — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md)); **оверлей статусов нод на канвасе** по потоку **`run-event`** (live/replay) — **сделано**, там же (**F13**); фаза **8** — надёжная доставка событий (**WebSocket** / буфер) и мост UI↔Python; внешний OTel — только при явном требовании **Aura** или enterprise.
...
| **GraphCaster** | **@xyflow/react** | Канвас, инспектор, предупреждения, P1/P2, **MiniMap** / поиск / ПКМ-меню / мультивыбор, **большие графы** (волна 1), оверлей **F13** — реестр в [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (**«MiniMap…»**, **«Поиск…»**, **«Меню «Добавить ноду»…»**, **«Мультивыбор…»**, **«Canvas: большие графы»**, **«Консоль наблюдаемости…»**, **«Открытие графа…»** и др.). В **§28.2** — только планирование **остатка** (lazy **A**, группы) | **`GraphDocument`** + **`validate.py`**; без БД (**F23**) |
...
4. **Мини-карта / навигация:** **MiniMap** / **Controls** и поиск на полотне — факты реализации только в [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (разделы **«MiniMap и панель управления полотном»**, **«Поиск и переход к ноде на canvas»**). Подсветка нод по событиям прогона (**оверлей** live/replay) — **F13**, там же. **Производительность очень больших графов** (viewport **React Flow**, батчинг оверлея, инкрементальный sync рёбер) — **«Canvas: большие графы»** в том же файле. **Открыто:** **lazy**-подграфы в схеме **A** / группы в документе (**п.7** ниже); дополнительная виртуализация DOM ноды вне кадра — по профилю (**§17**).
...
**Точечные углубления позже:** сравнение **z-index** и hit-testing **n8n** vs RF при 500+ нодах; паритет с **Dify** GraphEngine по **богатству** визуальных состояний ноды (ожидание / детализация barrier-**merge** и т.д.) — см. открытые пункты **F13** в **§17** и [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (**оверлей v1** уже есть).
...
3. **Межпрогоновый** параллелизм и полный паритет **n8n** после fan-out (очередь прогонов, queue mode, произвольные топологии веток вне ограниченного класса в **Merge**) — **§13** / **F6**; детали реализованного **внутриграфового** среза — только [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md).
