...
| **GraphCaster** | **частично** | Слой **B**: **@xyflow/react**, **`GcFlowNode`** / **`GcCommentNode`** / **`GcGroupNode`**, инспектор, предупреждения — **закрытые** срезы (**F18**, undo/redo, run-lock, sanitize, мини-карта, поиск, ПКМ-меню, мультивыбор, **производительность больших графов** волна 1, **рамки `group`**, F13 оверлей, **CI**) — детали мини-карты (в т.ч. **цвет по типу ноды**, **`minimapNodeColors.ts`**) только в [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (**«MiniMap…»** и др.: **«Поиск…»**, **«Меню «Добавить ноду»…»**, **«Мультивыбор…»**, **«Canvas: большие графы»**). **Остаток F1 / §28.2:** **lazy**-подграфы в схеме **A**; привязка к сетке и массовое выравнивание выделения — план **`doc/plans/2026-03-29-canvas-snap-and-align-plan.md`** (пока не в коде); опционально «лёгкий» хром ноды вне вьюпорта после профиля; полная типизация пинов — **§15**. |
