...
| **GraphCaster** | **частично** | Слой **B** (**полотно, инспектор, предупреждения**, **@xyflow/react**, ноды **`GcFlowNode`** / **`GcCommentNode`** / **`GcGroupNode`**): закрытые пункты **F1** / **§28.2** (канвас, **MiniMap**, поиск, clipboard, **group**, **snap/align**, **LOD**, **off-viewport ghost**, **ленивое превью `graph_ref`**, **F13**, открытие/Save …) — **единый SSOT** [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (**«Canvas: большие графы»** и смежные разделы); здесь перечень не дублировать. **Остаток F1:** расширенная типизация (**поля kind в JSON**, мультишины — **§15.2**); базовый **`PortDataKind`** фаза 1 — в том же [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (**F18**). **§29** (встраивание дочернего графа в **A** вне file-first). |
