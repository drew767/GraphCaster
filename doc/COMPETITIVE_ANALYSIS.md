...
**Реализованный слой:** консоль (**F13**, фаза 7) и **файловый журнал прогона** (**`events.ndjson`**, **`run-summary.json`**, UI **History**, брокер/Tauri) — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md). Ниже — **сравнение с конкурентами** и темы **F13**, которые в **`graph-caster`** остаются **вне** этого файла (**OTel**, ORM-**`Execution`**, транспорт **§39** / буфер **§39.2**).
...
**Точечные углубления позже:** сравнение с bounded queues в других стэках; лимиты прокси на длину SSE-чанка; политика при `QueueFull` в Python-мосте (кроме уже описанного dev-брокера в [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md)); полный **v2 workflow** stream в backend Langflow vs **LFX** — при сближении с продуктовым Langflow HTTP.
