...
| **GraphCaster** | **частично** | Слой **D**: обход **`start`**→**`exit`**, **F4** (**§32**) — реализация в коде: см. [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (ветвление по **`edge.condition`**); вложение **`graph_ref`** (**§29**); **достижимость / мёртвые ноды** — **§31.2**; циклы — **A** + **`validate.py`**. |
...
| **GraphCaster** | **частично** | **D+B**, **§32**: рантайм ветвления и контекст условий — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md); UI — **`branchWarnings`**. Нет паритета с выражениями **n8n**, **fail-ветками** (**§16**/**F19**), полным **F11** — `DEVELOPMENT_PLAN.md` / **§23**. |
...
| **GraphCaster** | Поле **`condition`** на **`Edge`** в **A** | **`_pick_next_edge`** + **`eval_edge_condition`** (`runner.py`, `edge_conditions.py`): первое ребро без условия **или** первое с истинным условием; иначе **`run_end`** **`no_outgoing_or_no_matching_condition`** | Legacy + JSON Logic-подмножество + **`node_outputs.*.processResult`**; детали и ограничения — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (**F4**), не дублировать здесь |
...
**Сделано в репозитории (факты и пути — не дублировать здесь):** инвариант «первое подходящее», безопасный DSL условий (**JSON Logic**-подмножество), **`node_outputs` / `processResult`**, лимит длины строки условия, связка UI **`branchWarnings`** — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (**F4**), **`python/README.md`**, **`edge_conditions.py`**, **`process_exec.py`**.

**Открыто / позже:**

1. **Ошибки и альтернативные ветки:** **`FAIL_BRANCH`** / **on_error** (**§16**) — отдельный контракт; не смешивать с условием на ребре, пока в **A** нет явного ошибочного исхода.
2. **Fork/join:** несколько живых веток с **merge** — вне текущей модели (**§31.2**); условные рёбра не делают **n8n**-merge сами по себе.
...
4. **События:** явные **`branch_taken`** / **`branch_skipped`** — **§3.7** + **§8**; сейчас основной сигнал — **`edge_traverse`**.
