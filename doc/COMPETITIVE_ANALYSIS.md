...
**Дата:** 2026-03-29 · **итерация 69** (**§29**: опциональный subprocess для **`graph_ref`** вынесен в [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md); **§32.2**: узкий **`$node`** в шаблонах и **`var`**; «открыто» — полный **n8n Expression** / JS sandbox; **§3.2.1** / **§39**: dev **WebSocket** + **`run_transport`** — факты в [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) **«Dev WebSocket и `run_transport`»**; **оверлей после выхода воркера** — строка **«Итог прогона на холсте…»** в том же [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md); **SQLite-каталог прогонов** — раздел **«Каталог прогонов (SQLite)»** там же; выполненные планы **`doc/plans/2026-03-29-run-execution-catalog-sqlite.md`**, **`doc/plans/2026-03-29-workspace-execution-history-ui.md`** удалены)  
...
| **GraphCaster** | **частично** | Слой **B** (**полотно, инспектор, предупреждения**, **@xyflow/react**, ноды **`GcFlowNode`** / **`GcCommentNode`** / **`GcGroupNode`**): закрытые пункты **F1** / **§28.2** (канвас, **MiniMap**, поиск, clipboard, **group**, **snap/align**, **LOD**, **off-viewport ghost**, **ленивое превью `graph_ref`**, **F13**, открытие/Save …) — **единый SSOT** [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (**«Canvas: большие графы»** и смежные разделы, **в т.ч. подписи на рёбрах** F4 / **`ai_route`** — **§28.2** п.8); здесь перечень не дублировать. **Остаток F1:** мультишины и прочее — **§15.2**. Типизация портов (**`PortDataKind`**, в т.ч. **`edges[].data.sourcePortKind` / `targetPortKind`**) — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (**F18**). **§29** (встраивание дочернего графа в **A** вне file-first). |
...
| **GraphCaster** | **F18:** контракт handle **`node.type`** + **`PortDataKind`** (таблица + опциональные переопределения на ребре в **`edges[].data`**); предупреждения kind в UI и **`structure_warning`** в раннере (без **`GraphStructureError`** на kind) | UI: **`handleContract.ts`**, **`portDataKinds.ts`**, **`portDataKindCompat.ts`**, **`handleCompatibility.ts`**, **`structureWarnings.ts`**, **`branchWarnings.ts`**; Python: **`handle_contract.py`**, **`port_data_kinds.py`**, **`validate.py`**; схема: **`graph-document.schema.json`** `$defs.edgeData` | Факты реализации — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (**F18**). Невыполненный хвост типизации — **§15.2** ниже. |
...
9. **Co-edit (**F22**):** не синхронизировать сырой internal-state **React Flow** по сети — только **документ** (**§19**).
