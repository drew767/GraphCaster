...
| **GraphCaster** | **частично** | Слой **B** (**полотно, инспектор, предупреждения**, **@xyflow/react**, **`GcFlowNode`** / **`GcCommentNode`** / **`GcGroupNode`**): закрытые срезы и факты по **F1** / производительности canvas — только [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md), раздел **«Canvas: большие графы»** (в т.ч. **ленивое превью `graph_ref`** в инспекторе) и смежные пункты (**MiniMap** в т.ч. **цвет по типу ноды**, **`minimapNodeColors.ts`**, поиск, ПКМ, мультивыбор, **snap-to-grid** / **align–distribute**, **LOD по zoom**, **F13**, **`group`** и т.д.). **Остаток F1 / §28.2:** опционально более лёгкий хром **полностью** вне вьюпорта (**§17**, профиль); полная типизация пинов — **§15**; **встраивание** дочернего графа в JSON родителя (**A**) вне file-first — **§29**. |
