...
**Дата:** 2026-03-29 · **итерация 69** (**§29**: опциональный subprocess для **`graph_ref`** — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md); **§32.2**: узкий **`$node`** / **`var`**; «открыто» — полный **n8n Expression** / JS sandbox; **§3.2.1** / **§39**: dev **WebSocket** + **`run_transport`** — **«Dev WebSocket и `run_transport`»** там же; **оверлей после выхода воркера** — **«Итог прогона на холсте…»** там же; **§15** / **F18**: перечень файлов и **`mergeGraphEdgeData`** — раздел **«Статическая совместимость ручек F18»** в [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md); локальные черновики **`doc/plans/`** — в **`.gitignore`**, после внедрения удалять только на диске)  
...
| **GraphCaster** | **F18:** контракт handle **`node.type`** + **`PortDataKind`** (таблица + опциональные переопределения в **`edges[].data`**); предупреждения kind в UI и **`structure_warning`** в раннере (без **`GraphStructureError`** на kind) | Статика до run: UI (**`findHandleCompatibilityIssues`** и др.), Python (**`validate_graph_structure`**, **`port_data_kinds`**, при run — **`find_port_data_kind_warnings`**) | Перечень модулей, тестов и **`mergeGraphEdgeData`** — раздел **«Статическая совместимость ручек F18»** в [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md). Открытый хвост типизации — **§15.2** ниже. |
