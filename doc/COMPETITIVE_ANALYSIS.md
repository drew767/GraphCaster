...
| **GraphCaster** | **частично** | Слой **B**: **@xyflow/react**, кастомные **`GcFlowNode`** / **`GcCommentNode`**, **`normalizeHandles`**, инспектор, предупреждения структуры; **совместимость ручек F18** (MVP) — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md); **undo/redo**, **run-lock** — там же (**F20**); **сериализация без связей «в никуда»** (**P2** `sanitizeGraphConnectivity`) — там же; **MiniMap** + **Controls** в MVP полотна — там же / **§28**; **§28.2** — производительность очень больших графов, поиск ноды; **CI** pytest/UI в монорепо **Aura** — там же; полная типизация пинов как у конкурентов — **§15**. |
...
4. **Мини-карта / навигация:** виджет **MiniMap** из **@xyflow/react** уже в MVP полотна (см. **`DEVELOPMENT_PLAN.md`**, [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md)); **открыто** на будущее: поиск ноды по имени, **виртуализация** или **lazy**-подграфы на больших графах только после профилирования (**§17** для потока событий — отдельно).
