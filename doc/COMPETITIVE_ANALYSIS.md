...
| **Vibe Workflow** | — | — | нет |
| **GraphCaster** | **частично:** **`task`**, **`mcp_tool`** и **`ai_route`** + диск + **`gcPin`** + **десктоп:** тоггл **Step cache**, очередь **dirty** (транзитивно по успешным рёбрам → **`--step-cache-dirty`**, как **`dirtyNodeNames`** у n8n) + **bubble** при правках во вложенном **`graph_ref`** (стек навигации, **`_parent_graph_ref_node_id`** в раннере) — см. [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md); **полный** F17 — **§22.2** | Ключ: ревизия JSON + отпечаток предков + **`data`** ноды + вид ноды (**`nk`**) — см. [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) | **partial**; **`gcPin`** + межпрогонный кэш **`task`** / **`mcp_tool`** / **`ai_route`** под **`artifacts_base`** — **да** (CLI и Tauri); кэш прочих типов нод / TTL — **нет** (**§22.2**) |
...
3. **`§22.2` — что ещё не закрыто относительно «полного» n8n/Comfy:** pin для всех типов исполняемых нод (не только **`task`**); step-cache для нод **кроме** **`task`**, **`mcp_tool`** и **`ai_route`**; отдельный продуктовый тоггл «зафиксировано / живое» не только через **`gcPin.enabled`**. **Закрыто в [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md):** межпрогонный step-cache **`mcp_tool`** (**`data.stepCache`**, **`fromStepCache`**, отключение при **`mcp_tool_provider`**); межпрогонный step-cache **`ai_route`** (**`data.stepCache`**, **`validate_ai_route_step_cache_entry`**, **`node_cache_hit`** без повторного **`ai_route_invoke`**, кэш действует и при **`ai_route_provider`** в контексте). Транзитив **`dirty`** при правках во вложенном **`graph_ref`** — **реализовано**, канон — абзац **«Вложенный `graph_ref`»** перед таблицей F17 там же. **Связь с артефактами (E):** **`--context-json`**, **`gcPin`**, **`node_outputs_snapshot`** — там же.
