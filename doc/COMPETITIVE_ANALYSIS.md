...
**Дата:** 2026-03-29 · **итерация 67** (**§29**: опциональный subprocess для **`graph_ref`** вынесен в [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md); **§32.2**: узкий **`$node`** в шаблонах и **`var`**; «открыто» — полный **n8n Expression** / JS sandbox; **§3.2.1** / **§39**: dev **WebSocket** + **`run_transport`** — факты в [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) **«Dev WebSocket и `run_transport`»**, черновик плана `doc/plans/2026-03-29-…` снят после закрытия фаз 0–3,5)  
