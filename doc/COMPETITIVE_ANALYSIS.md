...
| **GraphCaster** | **@xyflow/react** | Канвас, инспектор, предупреждения, P1/P2; MiniMap / поиск / ПКМ-меню / мультивыбор; оверлей **F13**; **открытие / Save / консоль** и др. — см. [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md). **Canvas (производительность, `group`, snap-to-grid, align/distribute)** — подпараграфы в разделе **«Canvas: большие графы»** того же файла (**без** дублирования здесь). В **§28.2** — планирование **остатка** только по **lazy**-подграфам в схеме **A** | **`GraphDocument`** + **`validate.py`**; без БД (**F23**) |
