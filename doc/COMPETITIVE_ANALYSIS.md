...
| **GraphCaster** | **частично** | **D+B**, **§32**: рантайм ветвления, DSL условий (**`$json`**, **`$node`**, шаблоны, JSON Logic) — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (**F4**); UI — **`branchWarnings`**. **Без** полноценного **n8n Expression** (JS sandbox). In-graph **`out_error`** (**F19**). Срез **F11** (делегированный агент / полный in-runner цикл) — таблица **F11** ниже и **§23**. |
...
| **GraphCaster** | **частично** | Реализованные срезы **F11** (**`ai_route`**, **`llm_agent`**) — только факты и пути в [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (разделы «ИИ-ветвление», «Делегированный LLM-агент») и **`python/README.md`**. Полный **F11** внутри раннера уровня **Dify**/**n8n** (ReAct, память, встроенный tool-calling) — **нет**; план и сравнение — **§23**. |
