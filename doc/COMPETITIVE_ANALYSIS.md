...
**Для GC:** не копировать объём **`ExecutionPushMessage`** и политику redaction; у n8n «живой» прогон — **отдельный канал** с **`pushRef`**, не REST. **Факты реализации GC** (NDJSON, **`runId`**, границы прогона, отмена, dev **SSE** `serve`; отличие канала от **`/push`**) — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md): «Сессия прогона…», «Реестр…», **«Десктоп…»** (в т.ч. строка **«Обрыв воркера»** — нормализация финала при обрыве дочернего процесса), **«Веб без Tauri»**, **«Backpressure SSE»** и **Evidence**. **Открыто:** паритет n8n — единый **WebSocket** или прод-**SSE** с **`pushRef`**, типами **`ExecutionPushMessage`**, relay при вынесенном воркере — **§39** (ср. Flowise **§3.3.1**, **§39.2** п.7).
...
| 9 Cursor CLI MVP | D, F | F7, **F8** | **Сделано (MVP пресета):** [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (**«Пресет Cursor Agent CLI»**). Эталоны и риски (**n8n** **Execute Command**, секреты **§11** / **§35**) — **§27** / **§11** (контракт **`gcCursorAgent`** только в **IMPLEMENTED_FEATURES**). |
...
1. **Фаза 9 (CLI) — реализовано в GC:** соглашение «секреты из OS env / `.env` не в репозитории», пресет **`gcCursorAgent`** без ключей в **`GraphDocument`** — факты в [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md); расширение до имён env в полях **`task`** без значений в JSON — по мере ТЗ (**Aura**), аналог Langflow «global var → env».
...
| **GraphCaster** | **`task`**: **`subprocess.Popen`** + pipe (**stdout**/**stderr**); **`command`/`argv`** или пресет **`gcCursorAgent`** (Cursor Agent CLI — см. [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md)); **`shlex.split`** (POSIX vs Windows), **`cwd`**, **`env`**, таймаут, усечённый вывод, **`successMode`**, ретраи (**§16**) | Доверие к автору графа и к хосту; секреты только через **§11** | **`process_exec.py`**, **`cursor_agent_argv.py`**, ветка **`task`** в **`runner.py`**, **`graph-document.schema.json`** |
...
1. **Фаза 9 (Cursor CLI MVP) — сделано:** пресет **`data.gcCursorAgent`**, резолвер **`GC_CURSOR_AGENT`**, сборка **`argv`** под **`agent -p`**, **`successMode`** на ноде **`task`** — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md), **`python/README.md`**. **Дальше (Aura / SaaS):** allowlist бинарей, жёсткая политика **cwd** — п.2–3 ниже.
