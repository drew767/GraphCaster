...
| **GraphCaster** | **@xyflow/react** | **`GcFlowNode`**, **`GcCommentNode`**, предупреждения структуры/ветвления; **`sanitizeGraphConnectivity`** при экспорте (**P2** — см. [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md)); инспектор; **MiniMap** / **Controls**, палитра «перейти к ноде», **ПКМ → добавить ноду** (чипы категорий) — тот же файл (**«MiniMap…»**, **«Поиск…»**, **«Меню «Добавить ноду»…»**); мультивыбор, **Ctrl+C/V** подграфа, групповое удаление, защита **`start`** — раздел **«Мультивыбор…»** | **`GraphDocument`** + **`validate.py`**; без БД (**F23**) |
...
7. **Комментарии и группы:** **`GcCommentNode`** уже задаёт прецедент «неисполняемая нода»; группы (frame) — только если появятся в схеме **A** и фильтре **`validate.py`**.
