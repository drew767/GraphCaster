...
| **GraphCaster** | **частично** | Слой **B**: **@xyflow/react**, **`GcFlowNode`** / **`GcCommentNode`** / **`GcGroupNode`**, инспектор, предупреждения — **закрытые** срезы (**F18**, undo/redo, run-lock, sanitize, мини-карта, поиск, ПКМ-меню, мультивыбор, **производительность больших графов** волна 1, сетка/align в **«Canvas: большие графы»**, **ленивое превью `graph_ref`** в инспекторе, **рамки `group`**, F13 оверлей, **CI**) — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md). **Остаток F1 / §28.2:** опционально «лёгкий» хром ноды вне вьюпорта после профиля; полная типизация пинов — **§15**; **встраивание** дочернего графа в JSON родителя (**A**) вне file-first — **§29**. |
...
1. **Не смешивать фазы:** фаза **7** — фильтры, поиск, экспорт из консоли, привязка к ноде (**сделано** — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md)); **оверлей статусов нод на канвасе** по потоку **`run-event`** (live/replay) — **сделано**, там же (**F13**); фаза **8** — надёжная доставка событий (**WebSocket** / буфер) и мост UI↔Python; внешний OTel — только при явном требовании **хост** или enterprise.
...
| **GraphCaster** | **@xyflow/react** | Канвас, инспектор, предупреждения, P1/P2, **MiniMap** / поиск / ПКМ-меню / мультивыбор, **большие графы** (волна 1, сетка/align, **ленивое превью `graph_ref`**), оверлей **F13**, **рамки `group`** / **Group selection** — реестр в [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (**«MiniMap…»**, **«Поиск…»**, **«Меню «Добавить ноду»…»**, **«Мультивыбор…»**, **«Canvas: большие графы»**, **«Консоль наблюдаемости…»**, **«Открытие графа…»** и др.). В **§28.2** остаётся опциональная виртуализация DOM и **§15**; **встраивание** дочернего JSON в **A** — вне file-first (**§29**) | **`GraphDocument`** + **`validate.py`**; без БД (**F23**) |
...
4. **Мини-карта / навигация:** **MiniMap** / **Controls** и поиск на полотне — факты реализации только в [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (разделы **«MiniMap и панель управления полотном»**, **«Поиск и переход к ноде на canvas»**). Подсветка нод по событиям прогона (**оверлей** live/replay) — **F13**, там же. **Производительность очень больших графов** (viewport **React Flow**, батчинг оверлея, инкрементальный sync рёбер) — **«Canvas: большие графы»** в том же файле. **Ленивое превью `graph_ref`** в инспекторе (чтение дочернего JSON с диска только после явного раскрытия; кэш в **`AppShell`**) — закрыто там же. **Открыто:** дополнительная виртуализация DOM ноды вне кадра — по профилю (**§17**). **Встраивание** полного дочернего графа в документ родителя (**A**) по-прежнему вне scope — **§29** (file-first).
