...
4. **Мини-карта / навигация:** **MiniMap** / **Controls** и поиск на полотне — факты реализации только в [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (разделы **«MiniMap и панель управления полотном»**, **«Поиск и переход к ноде на canvas»**). Подсветка нод по событиям прогона (**оверлей** live/replay) — **F13**, там же. **Открыто:** **виртуализация** или **lazy**-подграфы после профилирования; производительность оверлея/ресинка на очень больших графах — при необходимости отдельный профиль (**§17**).
