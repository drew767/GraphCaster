...
**Уже реализовано и вынесено из «пробела» относительно конкурентов:** см. [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (host/run state, жизненный цикл NDJSON с `runId`, **десктоп:** мост UI ↔ Python через Tauri subprocess; **частичный прогон** — **`--until-node`**, **`run_finished.status` `partial`**, **`--context-json`** — раздел «Частичный прогон»; **закреплённый вывод `gcPin`** (UI + раннер, аналог n8n **`pinData`** для **`task`**) — подраздел «Закреплённый вывод…»; **межпрогонный кэш `task`** — **`--step-cache`**, **`--step-cache-dirty`**, **`node_cache_*`**, в десктопе — тоггл Run и очередь **dirty** — подраздел «Межпрогонный кэш выходов **task**»; **CI** монорепо **Aura** для pytest + UI build — отдельный подраздел там же).
...
| **GraphCaster** | **частично** | **D+B**, **§32**: рантайм ветвления и контекст условий — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md); UI — **`branchWarnings`**. **MVP-шаблоны** **`{{ dotted.path }}`** плюс зарезервированный корень **`$json`** (конверт **`last_result`**, без JS VM) — там же; **без** полноценного **n8n Expression** / произвольного **`$node[…]`**. In-graph **`out_error`** после сбоя **`task`** / **`graph_ref`** — там же (**F19**). Нет паритета с полным expression runtime **n8n**, отдельным **error-workflow** как у **n8n**, полным **F11** — `DEVELOPMENT_PLAN.md` / **§23**. |
...
| **GraphCaster** | **частично** | **Факты в коде** — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md): «Частичный прогон», «Межпрогонный кэш выходов **`task`**», «Закреплённый вывод (**`gcPin`**)». Здесь только сравнение с эталонами. **Зазор к n8n Comfy-grade F17:** тоггл step-cache на полотне, кэш произвольных нод (не только **`task`**), TTL/лимиты, **`dirtyNodeNames`** в UI как при **`runPartialWorkflow2`** — **§22.2**, **§36**. |
...
**Срез в GC:** partial; **`gcPin`** в документе + раннер (**`node_pinned_skip`**, **`node_outputs_snapshot`**) и UI для **`task`** — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) («Закреплённый вывод…»); ручной контекст **`--context-json`**; **headless** и **десктоп** межпрогонный кэш **`task`** (**`--step-cache`**, **`--step-cache-dirty`**, ревизия документа, события **`node_cache_*`**) — там же («Межпрогонный кэш…», строка про Tauri). Ниже — сравнение конкурентов и **остаточный** план до паритета с полным Comfy/n8n (**не** дублировать таблицу реализации здесь).
...
| **Vibe Workflow** | — | — | нет |
| **GraphCaster** | **частично:** **`task`** + диск + **`gcPin`** + **десктоп:** тоггл **Step cache** и очередь **dirty** → **`--step-cache-dirty`** (см. [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md)); **полный** F17 — **§22.2** | Ключ: ревизия JSON + отпечаток предков + **`data`** ноды (см. [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md)) | **partial**; **`gcPin`** + межпрогонный кэш **`task`** под **`artifacts_base`** — **да** (CLI и Tauri); автоматическое замыкание **dirty** по DAG / кэш остальных типов нод / TTL — **нет** (**§22.2**) |

### 22.2. Планирование для GC (остаток после headless-кэша **`task`**)

Реализованный срез (**`task`**, диск, **`context["tenant_id"]`** в ключе при необходимости) — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md). Дальше — что остаётся до «полного» F17 в смысле Comfy/n8n.
...
3. **Связь с артефактами (E):** **`--context-json`**, документный **`gcPin`** и снимок **`node_outputs_snapshot`** → «pin из последнего запуска» в десктопе — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md). **Step cache / dirty** в десктопе — там же (очередь **`--step-cache-dirty`**). Остаётся до полного n8n: транзитивное **`dirtyNodeNames`** по графу без ручного списка, pin для всех типов исполняемых нод, отдельный продуктовый тоггл «зафиксировано / живое» не только через **`gcPin.enabled`** — **§22.2**.
