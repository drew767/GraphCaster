...
| **GraphCaster** | **частично** | **Факты в коде** — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md): «Частичный прогон», «Межпрогонный кэш выходов **`task`**» (в т.ч. **ревизия вложенного **`graph_ref`** в ключе**), «Закреплённый вывод (**`gcPin`**)», транзитивная очередь **dirty** (успешные рёбра). **Зазор к n8n Comfy-grade F17:** тоггл step-cache на полотне, кэш произвольных нод (не только **`task`**), TTL/лимиты — **§22.2**, **§36**. |
