...
|------|-----------------|------------------------|
| **A. Документ и схема** | JSON Schema, `GraphDocument`, версии, инварианты графа; контракт строк раннера | `schemas/graph-document.schema.json`, **`schemas/run-event.schema.json`**, `models.py`, `validate.py`; **`schemaVersion`** и миграции документа — **§30** (**F2**); **F18** (ручки + фаза 1 **`PortDataKind`**) — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md); хвост типизации (поля kind в JSON, **`blob_ref`**, мультишины, медиа-сокеты) — **§15**; новые **`kind`** нод — **§18** (**F15**) |
...
| **GraphCaster** | **частично** | Слой **B** (**полотно, инспектор, предупреждения**, **@xyflow/react**, ноды **`GcFlowNode`** / **`GcCommentNode`** / **`GcGroupNode`**): закрытые пункты **F1** / **§28.2** (канвас, **MiniMap**, поиск, clipboard, **group**, **snap/align**, **LOD**, **off-viewport ghost**, **ленивое превью `graph_ref`**, **F13**, открытие/Save …) — **единый SSOT** [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (**«Canvas: большие графы»** и смежные разделы, **в т.ч. подписи на рёбрах** F4 / **`ai_route`** — **§28.2** п.8); здесь перечень не дублировать. **Остаток F1:** расширенная типизация (**поля kind в JSON**, мультишины — **§15.2**); базовый **`PortDataKind`** фаза 1 — в том же [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (**F18**). **§29** (встраивание дочернего графа в **A** вне file-first). |
...
| **GraphCaster** | **частично** | **A+B:** именованные ручки **`in_default`/`out_default`/`out_error`**; контракт по **`node.type`** + **фаза 1** **`PortDataKind`** (предупреждения kind; жёсткая **`validate_graph_structure`** только на неверных id ручек) — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (**F18**). **Остаток к эталонам:** поля kind в JSON, **`blob_ref`**, мультишины **n8n** — **§15.2**. |
...
| **GraphCaster** | **MVP F18 + фаза 1 kind:** контракт handle **`node.type`** + **`PortDataKind`**; предупреждения kind в UI и **`structure_warning`** в раннере (без **`GraphStructureError`** на kind) | UI: **`handleContract.ts`**, **`portDataKinds.ts`**, **`portDataKindCompat.ts`**, **`handleCompatibility.ts`**, **`structureWarnings.ts`**, **`branchWarnings.ts`**; Python: **`handle_contract.py`**, **`port_data_kinds.py`**, **`validate.py`** | Единый SSOT реализации — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (**F18**). Открытый хвост — **§15.2** ниже (не дублировать перечни путей здесь). |

### 15.2. Планирование для GC

Закрытые факты **F18** (имена ручек, фаза 1 **`PortDataKind`**, пути к коду и тестам, остаток типизации в одном абзаце) — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md), раздел **«Статическая совместимость ручек F18»**; в этом подразделе — только **что ещё не** сделано и идеи.

1. **Документ и расширение типов:** опциональные поля **`edges[].data`** / обновление **`graph-document.schema.json`** для переопределения kind; **`blob_ref`**; мультишины (**n8n**); **не копировать Comfy 1:1** (IMAGE/LATENT, …) без домена медиа. Новый **`kind`** ноды задаёт набор портов — **§18** (**F15**).
