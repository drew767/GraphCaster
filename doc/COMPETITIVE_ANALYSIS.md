...
**Срез в GC:** partial; **`gcPin`** в документе + раннер (**`node_pinned_skip`**, **`node_outputs_snapshot`**) и UI для **`task`** — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) («Закреплённый вывод…»); ручной контекст **`--context-json`**; **headless** межпрогонный кэш выходов **`task`** (**`--step-cache`**, ревизия документа, **`node_cache_hit`** / **`node_cache_miss`**) — там же («Частичный прогон», «Межпрогонный кэш…»). Ниже — сравнение конкурентов и **остаточный** план до паритета с полным Comfy/n8n (**не** дублировать таблицу реализации здесь).
...
| **Vibe Workflow** | — | — | нет |
| **GraphCaster** | **частично:** **`task`** + диск + **`gcPin`** (см. [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md)); **полный** F17 — **§22.2** | Ключ: ревизия JSON + отпечаток предков + **`data`** ноды (см. [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md)) | **partial**; **`gcPin`** + межпрогонный кэш **`task`** под **`artifacts_base`** — **да**; тоггл кэша на полотне / кэш остальных типов нод — **нет** (**§22.2**) |
...
3. **Связь с артефактами (E):** **`--context-json`**, документный **`gcPin`** и снимок **`node_outputs_snapshot`** → «pin из последнего запуска» в десктопе — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md). Остаётся до полного n8n: **`dirtyNodeNames`** на полотне, pin для всех типов исполняемых нод, отдельный продуктовый тоггл «зафиксировано / живое» не только через **`gcPin.enabled`** — **§22.2**.
