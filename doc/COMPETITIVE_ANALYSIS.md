...
**Для GC:** не копировать объём **`ExecutionPushMessage`** и политику redaction; у n8n «живой» прогон — **отдельный канал** с **`pushRef`**, не REST. **Факты реализации GC** (NDJSON, **`runId`**, границы прогона, отмена, dev **SSE** `serve`, отличие от **`/push`**) — только [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (разделы «Сессия прогона…», «Реестр…», «Десктоп…», «Веб без Tauri», «Backpressure SSE»). **Открыто:** паритет n8n — единый **WebSocket** или прод-**SSE** с **`pushRef`**, типами **`ExecutionPushMessage`**, relay при вынесенном воркере — **§39** (ср. Flowise **§3.3.1**, **§39.2** п.7).
...
| **GraphCaster** | **@xyflow/react** | **`GcFlowNode`**, **`GcCommentNode`**, предупреждения структуры/ветвления; **`sanitizeGraphConnectivity`** при экспорте (**P2** — см. [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md)); инспектор; **MiniMap** / **Controls**, палитра «перейти к ноде» — тот же файл (**«MiniMap…»**, **«Поиск…»**); мультивыбор, **Ctrl+C/V** подграфа, групповое удаление, защита **`start`** — раздел **«Мультивыбор…»** | **`GraphDocument`** + **`validate.py`**; без БД (**F23**) |
...
1. **Фазы 3–4:** довести **паритет по подсказкам** (ветвление и прочее) с минимальным когнитивным шумом; не дублировать **n8n** полноту интеграций — фокус на **`task`** и **`graph_ref`**. **Рёбра с отсутствующими концами** при сериализации — **закрыто** (**P2** `sanitizeGraphConnectivity`); **открытие / инспектор / Save / workspace** (ошибки JSON, имени файла, записи, конфликт **`graphId`**) — **закрыто** (P1): единая модалка и презентации — раздел **«Открытие графа: ошибки JSON и парсера»** в [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md). Остаётся жёсткость пинов **§15** / **§18** без полного копирования конкурентов; детали реализации P1/P2 в **§28** не дублируются.
...
4. **Мини-карта / навигация:** **MiniMap** / **Controls** и поиск на полотне — факты реализации только в [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (разделы **«MiniMap и панель управления полотном»**, **«Поиск и переход к ноде на canvas»**). **Открыто:** **виртуализация** или **lazy**-подграфы после профилирования; поток событий на очень больших графах — **§17** отдельно.
