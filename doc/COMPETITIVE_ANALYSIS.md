...
**Дата:** 2026-03-29 · **итерация 67** (**§29**: опциональный subprocess для **`graph_ref`** вынесен в [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md); **§32.2**: узкий **`$node`** в шаблонах и **`var`**; «открыто» — полный **n8n Expression** / JS sandbox; **§3.2.1** / **§39**: dev **WebSocket** + **`run_transport`** — факты в [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) **«Dev WebSocket и `run_transport`»**)  
...
| **GraphCaster** | **частично** | **Факты в коде** — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md): «Частичный прогон», «Межпрогонный кэш выходов **`task`** и **`mcp_tool`**» (в т.ч. **ревизия вложенного **`graph_ref`** в ключе**, поле **`nk`** в материале ключа), «Закреплённый вывод (**`gcPin`**)», транзитивная очередь **dirty** (успешные рёбра). **Зазор к n8n Comfy-grade F17:** тоггл step-cache на полотне, step-cache для **`ai_route`** и остальных типов нод, TTL/лимиты — **§22.2**, **§36**. |
...
**Реализованный слой:** консоль (**F13**, фаза 7) и **файловый журнал прогона** (**`events.ndjson`**, **`run-summary.json`**, UI **History**, брокер/Tauri) — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md); **dev run-брокер** (`serve`, **SSE** / **WebSocket**) и bounded backpressure — там же (**«Backpressure SSE»** + **Evidence**), без дубля в этом разделе. Ниже — **сравнение с конкурентами** и темы **F13**, которые в **`graph-caster`** остаются **вне** этого среза (**OTel**, ORM-**`Execution`**, **prod**-транспорт **§39** / relay **§39.2** п.7 и п.6 — политика хоста).
...
| **Vibe Workflow** | — | — | нет |
| **GraphCaster** | **частично:** **`task`** и **`mcp_tool`** + диск + **`gcPin`** + **десктоп:** тоггл **Step cache**, очередь **dirty** (транзитивно по успешным рёбрам → **`--step-cache-dirty`**, как **`dirtyNodeNames`** у n8n) + **bubble** при правках во вложенном **`graph_ref`** (стек навигации, **`_parent_graph_ref_node_id`** в раннере) — см. [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md); **полный** F17 — **§22.2** | Ключ: ревизия JSON + отпечаток предков + **`data`** ноды + вид ноды (**`nk`**) — см. [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) | **partial**; **`gcPin`** + межпрогонный кэш **`task`** / **`mcp_tool`** под **`artifacts_base`** — **да** (CLI и Tauri); кэш **`ai_route`** и прочих типов / TTL — **нет** (**§22.2**) |
...
3. **`§22.2` — что ещё не закрыто относительно «полного» n8n/Comfy:** pin для всех типов исполняемых нод (не только **`task`**); step-cache для нод **кроме** **`task`** и **`mcp_tool`** (например **`ai_route`**); отдельный продуктовый тоггл «зафиксировано / живое» не только через **`gcPin.enabled`**. **Закрыто в [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md):** межпрогонный step-cache **`mcp_tool`** (**`data.stepCache`**, **`fromStepCache`**, отключение при **`mcp_tool_provider`**). Транзитив **`dirty`** при правках во вложенном **`graph_ref`** — **реализовано**, канон — абзац **«Вложенный `graph_ref`»** перед таблицей F17 там же. **Связь с артефактами (E):** **`--context-json`**, **`gcPin`**, **`node_outputs_snapshot`** — там же.
...
4. **Fork/join (несколько предков):** в **A** — **`merge`** (**passthrough** / **`barrier`**) и **`fork`**; по умолчанию ветки **последовательно**; опционально **bounded OS-параллель** (лимит **`fork.data.maxParallel`**, CLI/env, пул потоков) для **ограниченного класса** топологий (линейные ветки с subprocess-**`task`** до одного barrier) — только факты в [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (**Merge**). Полный паритет **n8n Merge** (произвольные ветки, очередь прогонов) — **§13** / **F6**. До отдельного ТЗ не расширять семантику неявно (**PRODUCT_DESIGNE.md**).
...
3. **Параллелизм после fan-out vs полный **n8n** / worker pool:** **Внутри одного прогона** — опциональный **bounded OS-параллель** линейных веток **`fork` → `task` → … → `merge` `barrier`** (**`fork.data.maxParallel`**, CLI/env) — факты только в [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (подраздел **Merge (`join`)**); порядок **`emit`** между ветками не детерминирован как «порядок в JSON». **Межпрогоновый** queue mode / горизонтальные воркеры / полный паритет **n8n Merge** для произвольных топологий — **§13** / **F6**, вне текущего среза.
...
2. **Порядок зрелости (остаточный):** стабильный **HTTP run** (**§25**) и **`task`** (**§27**) остаются базой для **Aura**; углубление **(A)** (публичный MCP) — после контрактов **§20** / **§38**.
