...
| **G. Наблюдаемость** | Логи исполнения, история, трейсинг, экспорт | Консоль UI + NDJSON события (**§3.7**), артефакты **`runs/`**; мост **WebSocket/SSE** поверх того же потока — **§39** (ограничение буфера / медленный клиент — **§39.2**); открытые темы — **§17** (**F13**) |
...
Эти два фрагмента полезны при проектировании **фазы 8** GC (мост UI ↔ раннер) и будущего **RunHostContext**: не повторять объём кода, а понять **разделение данных графа и окружения исполнения**.
...
- **Для GC:** явно разделить (1) **документ графа** JSON, (2) **run state** (как `runExecutionData`), (3) **host** — пути workspace, артефакты, будущие credentials, мост к UI; это уменьшит смешивание «логики ноды» и «инфраструктуры запуска**.
...
**Для GC:** не копировать объём **`ExecutionPushMessage`** и политику redaction; достаточно помнить, что «живой» прогон в n8n — это **отдельный канал с сессионным **`pushRef`**, не тот же REST, что старт run**. Для фазы 8 GC ближе **один `runId` на событие** (**§3.7**, **§39**). Relay при вынесенном воркере — аналог по идее **§39.2** п.7 (ср. Flowise **§3.3.1**).
...
**Для GC:** публичный streaming execution для Langflow в open repo — ось **`§3.4.1`** / **`§3.4.2`**, не **v2 workflows**; при сравнении с **Aura BFF** не предполагать **`POST /api/v2/workflows`** + **`stream`** до смены кода upstream.
...
**Для GC:** при сравнении с Comfy как эталоном «живого» UI фиксировать, что **контракт сокета зависит от набора установленных нод**; для **Aura** предпочтительнее **закрытый enum `run-event`**, а не открытый список строк как у сторонних Comfy-нод.
...
**Для GC:** два разных HTTP-представления одного внутреннего кадра (**full** vs **simple**) — учитывать, если сравнивать сниффинг API Dify с логами отладчика; для собственного моста **Aura** достаточно одного контракта (**NDJSON + `type`**, **§3.7**).
...
**Для GC / планирования:** контракт «тонкий BFF + ключ в **`x-api-key`**» совпадает с **§25.3**; таблица даёт **проверяемую** картину **без** спецификации полей тел **run** / **status** у MuAPI — при интеграции **Aura** их нужно брать из документации провайдера или трассировки, не из **graph-caster**.
...
| **GraphCaster** | **частично** | Слой **B**: **@xyflow/react**, кастомные **`GcFlowNode`** / **`GcCommentNode`**, **`normalizeHandles`**, инспектор, предупреждения структуры; **§28.2** — мини-карта, undo, run-lock, производительность. |
...
| **GraphCaster** | **частично** | Слой **D**: обход от **`start`** к **`exit`**, **упорядоченный список исходящих рёбер** для условий (**F4**, семантика — **§32**); вложение **`graph_ref`** (**§29**); **достижимость / мёртвые ноды** — **§31.2**; циклы в одном файле — политика **A** + **`validate.py`**. |
...
| **GraphCaster** | **частично** | Слой **D+B**: **`edge.condition`** + **`_pick_next_edge`** в **`runner.py`** (**§32**); контекст **`last_result`** для нестроковых литералов; UI — **`branchWarnings`**; выражения **n8n**-уровня, **fail-ветки** и полный **F11** — по плану (**§16**, **§23**, `DEVELOPMENT_PLAN.md`). |
...
| **GraphCaster** | **частично** | Слой **D+C**: **`kind`** **`graph_ref`**, **`graphId`**, загрузка из **`workspace.py`**, синхронный вложенный прогон; события nested; лимит глубины; циклы — валидация (**§29.2**). |
...
| **GraphCaster** | **нет (пока)** | Слой **D**: один синхронный прогон на процесс CLI; **§13** — целевая эволюция без копирования Redis-очереди n8n. |
...
| **GraphCaster** | **частично** | Слой **D**: нода **`task`** → **`process_exec.py`** (`command`/`argv` через **`shlex`**, **`cwd`**, **`env`**, таймаут, лимит stdout, **`successMode`**, ретраи, события **`process_*`**) — **§27.2**. |
...
| **GraphCaster** | **нет / план** | Слой **F**: маскирование секретов для AI — «на рассмотрении» в плане; пока `env` в задаче процесса локально. Подробнее: **§11** (vault), **§35** (допустимые поля в **`GraphDocument`** / git). |
...
| **GraphCaster** | **план** | **§23.2:** сначала **статическое** ИИ-ветвление (**фаза 6**); полноценный **F11** — отдельные **`kind`** или внешний orchestrator (не дублировать **Dify**/**n8n** в ядре). |
...
| **GraphCaster** | **нет** | **§20.2**, **§38:** идентичность и ACL — **Aura**; GC принимает уже отфильтрованный путь/`graphId` (**§10**). |
...
| **GraphCaster** | **нет** | MVP: полный обход; **§22.2** + **§36.2:** ключ кэша = `hash(kind + data + входы + graph revision)`, хранилище — под **`runs/`** или отдельный каталог; инвалидация при смене файла / **`schemaVersion`**. |
...
| **GraphCaster** | **нет (открыто)** | **`error`** + **`process_*`** в NDJSON (**§3.7**); явной `on_error` ветки в документе пока нет — **§16.2**, **§37.2**, тема **2b** в `DEVELOPMENT_PLAN.md`. |
...
| **GraphCaster** | **нет (план)** | **§21.2:** слой **B**, **фаза 5** — явные команды над `GraphDocument`, не «магия» React Flow. |
...
| **GraphCaster** | **нет** | **C:** локальный JSON (**§10**, **F23**); **F22** сознательно **вне** репозитория — хост (**Aura**) или **§19.2**. |
...
2. **Не брать в MVP** без явного решения: **F6** — **§13**; **F10** (ingestion + vector DB + раннер) — **§14**; **F14** (tenant, RBAC, SSO) — **§20** / **§38**, не дублировать Dify/n8n в `graph-caster`. **F8** — **§11** / **§35**. **F9**/**F12** — **§12**; триггеры — **§24**; публичный REST, ключи, OpenAPI — **§25**. **F22** (реалтайм co-edit, Yjs) — **§19**; не смешивать с **фазой 5** run-lock (локальная блокировка UX).
...
5. **Фаза 8 (мост):** у **Dify** в монолите буфер между GraphEngine и HTTP — **in-process** **`queue.Queue`** (**§3.6.1**), чтение в **`StreamResponse`** и **`break`** по финальным **`Queue*`** — **§3.6.2**; у **Flowise** в queue mode — **Redis** (**§3.3.1**). В протоколе событий GC закрепить **`runId`** (или эквивалент) на **каждой** строке NDJSON — аналог `prompt_id` в Comfy (**§3.5**) и **`workflow_run_id`** у Dify (**§3.6.1**–**§3.6.2**), плюс обход путаницы при нескольких сессиях (**§13**); внутренний маппинг «раннер → консоль» можно держать плоским, отдельный слой Queue* (**§3.6**) — только если появятся разные потребители одного прогона. Актуальный перечень `type` и пробелы — **§3.7**. Разделение **очереди исполнения** и **буфера к транспорту** (эталон Comfy — **§13.3**); при вынесении воркера — мост событий (эталон Flowise **Redis** — **§3.3.1**); явная политика при медленном клиенте — **§39.2**.
6. **Расширение логов `task` / CLI (**F7**):** при сценарии **Cursor CLI** опираться на **§27**; сравнить потоковый вывод с **Comfy `progress`** и **Langflow** SSE; не смешивать с **`node_execute`** (сейчас снимок полей `data`, не непрерывный stdout в событии).
...
8. **Наблюдаемость (**F13**):** различать **поток для UI** (консоль, фильтры — **§17**), **файлы артефактов** (**E**) и опциональный **экспорт в внешний трейсер**; транспорт консоли (**NDJSON** vs **WS/SSE**) — **§39** / **§39.2**; не дублировать Dify **`TraceQueueManager`** без продукта-получателя.
9. **Новый тип ноды (**F15**):** менять одновременно **A+B+D** — см. **§18**; не добавлять `kind` только в UI без **`validate.py`** и ветки в **`runner.py`**.
10. **Совместное редактирование (**F22**):** не внедрять CRDT внутрь graph-caster без решения **Aura**; см. **§19** (граница ответственности **C** vs хост).
...
12. **Undo/redo (**F20**):** стек команд в UI — **§21**; не путать с версионированием workflow в БД (**n8n** / **Dify**) и с **YjsUndoManager** при **F22** (**§19**).
13. **Кэш / инкремент (**F17**):** не смешивать кэш **индекса workspace** (`clear_graph_index_cache` в **`workspace.py`**) с кэшем **выходов нод** — **§22** / **§36**; Comfy-модель требует стабильных сигнатур входов; при **F23** смена JSON на диске должна менять **ревизию**, иначе кэш даст ложные попадания.
...
16. **Публичный API (**F12**):** отделять **внутренний мост** (фаза 8) от **внешнего** контракта; не отдавать целиком `graph-document` без ACL — **§25**; BFF без утечки секретов в браузер — **§25.3**; аутентификация и квоты — **Aura** (**§20**).
...
18. **Внешний процесс / CLI (**F7**):** не класть **секреты** в **`command`** / **`argv`** (**§11**); для SaaS (**Aura**) заранее продумать **allowlist** бинарей или политику **cwd**; таймауты и **лимит stdout** согласовать с консолью (**§17**) и артефактами (**E**) — **§27**; **`shlex`** уже различает Windows/POSIX в **`process_exec.py`**.
...
20. **Вложенные графы (**F5**):** **`graph_ref`** только на существующий **`graphId`** (**C**, **§10**); циклы и глубина — **`validate.py`** + раннер (**§29**); **`runId`** корневого прогона не «терять» во вложенных событиях (**§3.7**, **§17**); в Aura — ACL «какой граф можно вызывать» (**§20**, **§38**).
...
23. **Ветвление (**F4**):** смена **семантики условия на ребре**, **контекста** для вычисления (**`last_result`**, будущие поля), **порядка «первое подходящее»** относительно безусловного ребра или **событий** при несовпадении всех условий — **§32**; держать в синхроне **`graph-document.schema.json`**, **`models.py`**, **`branchWarnings`**, при необходимости **`run-event`** (**§3.7**); не путать с **DAG merge**/fork (**§31.2**) и с **`FAIL_BRANCH`** (**§16**, **F19**).
...
| 6 условия, ИИ-ветвление | B, D, F | F4, F11; **F19** (ветвление при сбое — позже, **§16** / **§37**) | **§32** (**F4**): семантика **`edge.condition`** и контекст; **§31** (**F3**) — только порядок списка рёбер; **Dify** **`SkipPropagator`** / условные ноды, **`FAIL_BRANCH`** (**§16**), **Flowise** condition nodes, **n8n** IF/Switch; полный **F11** — **§23** |
| 7 консоль (фильтры, экспорт) | G | F13 | **§17**; **Dify** **`ObservabilityLayer`** + слои persistence/trace; **n8n** список execution в БД; **Langflow** **`TracingService`** |
| 8 мост UI ↔ Python (NDJSON/WebSocket) | B, D, G | F12, F13 | Внутренний мост ≠ **§25** / **§34** (MCP — другой контур); обзор **§12**; ось транспорта **§39** / **§39.2**; два уровня очередей (исполнение vs сокет) — **§13.3**; стабильный **`runId`** — **§17.2**, **§3.7**, реализация — **§3.7.1**. Референсы: **ComfyUI** §3.5 / **§3.5.1** / §13.3, **n8n** §3.2.1 (Push + relay) / **§3.2.2** / **§3.2.3** / **§3.2.4** / **§3.2.5** (redaction + сервис + стратегии + инвентаризация полей), **Flowise** §3.3.1 / **§3.3.3**, **Langflow** §3.4 / §3.4.1 / **§3.4.2**, **Dify** §3.6 → Queue* → HTTP **§3.6.1** / **§3.6.2** / **§3.6.3**, **Vibe** **§3.8** / **§3.8.1** (BFF → poll) |
| 9 Cursor CLI MVP | D, F | F7, **F8** | **§27**; **n8n** **Execute Command** (риски RCE на хосте); секреты — **§11** / **§35** (env / хост, не literal в **`GraphDocument`**); пресеты **`argv`** под IDE — без хардкода путей пользователя в репозитории |
...
**Дальнейшие точечные углубления:** Comfy — инвентаризация **`send_sync`/JSON `type`** по **конкретной** установке (diff форка vs upstream; методика — **§3.5.1**); **Flowise** — выравнивание **`RedisEventPublisher.streamMetadataEvent`** с **`SSEStreamer`** (или осознанная политика урезания) — поверх **§3.3.3**; Dify — OpenAPI / сопоставление маршрутов с веткой **`InvokeFrom`** (**§3.6.3**); n8n — при смене версии субмодуля повторить **`sensitiveOutputFields`** по **`packages/nodes-base`** и при необходимости по закрытым пакетам билда (**§3.2.5** — снимок open tree); Vibe — тела ответов **run/status** MuAPI (вне open repo); каталог прокси **BFF → MuAPI** — **§3.8.1**; **`runId`** в GC — план **§3.7.1**, после реализации снять пометку «резерв» в **JSON Schema**; публичная схема **Queue*** в OpenAPI Dify — только если интегрироваться с Dify, а не с собственным раннером GC.
...
**Вывод для GC:** текущие конкуренты (кроме Comfy и частично Langflow export) заточены под **один сервер — много пользователей/проектов** и **БД**. Модель GC (**§F23**) ближе к **репозиторию кода + локальный CLI**: проще дифф в git, проще передать путь в Cursor, сложнее без доработки получить «одна кнопка SaaS». При встраивании в **Aura** не подменять file-first без решения продукта: можно пополнять индекс из облака, но канонический источник по плану — папка **`graphs/`**. Если два редактора пишут один и тот же файл — политика слияния и real-time — **§19** (**F22**), не дублировать в **`workspace.py`** без явного ТЗ.
...
| **GraphCaster (цель)** | **Пока:** `env` в **`task`** / окружение процесса; **не** хранить ключи в committed JSON | **План:** только **имена** переменных или **handlebars**-подобные ссылки без значений в документе | Раннер или **Aura** подставляет значения перед `subprocess`; маскирование в событиях (**G**) — по мере появления чувствительных полей в `node_execute` |

**Рекомендации для планирования GC (без раздувания MVP):**

1. **Фаза 9 (CLI):** оставить соглашение «секреты из OS env / `.env` не в репозитории»; в `task` допускать только **имена** ключей env (`CURSOR_API_KEY` и т.д.), как у Langflow «сначала global var, потом env».
2. **Фаза 10 (Aura):** если появится центральный vault — резолв **на границе** (HTTP header / one-shot env dict в вызов раннера), аналог подмешивания Dify/n8n, без обязательной таблицы внутри репо GC.
...
4. **Observability:** при логировании `node_execute` (§3.7) не включать развёрнутые секреты; для AI-ассистентов — политика маскирования как отдельный подпункт **F8** после появления полей `secret` в схеме.
...
| **GraphCaster** | **План:** не встраивать cron как n8n в MVP | **Сейчас:** `python -m graph_caster`; **фаза 8:** мост UI↔раннер; **фаза 10:** тонкий HTTP от **Aura** или обёртка, вызывающая тот же CLI/библиотеку | Не плодить второй «BaaS»; один контракт: `graphId` + опции run + **§11** env |

### 12.2. Планирование для GC

1. **F9 без дубля n8n:** не обязательно добавлять ноду «Cron» в canvas. Достаточно, чтобы **хост** (Kubernetes CronJob, systemd timer, **Aura** scheduler) вызывал **`graph_caster` с `graphId`** или путём к JSON — аналогично тому, как **Langflow** часто запускают извне.
2. **Webhook:** если понадобится, разумная модель — **одна** защищённая конечная точка на стороне **Aura** (auth, rate limit, tenant), которая парсит payload и дергает раннер; не смешивать с редактированием графа в **§B**.
...
| **GraphCaster** | **Сейчас:** один процесс, один прогон; параллельно — только несколько CLI/ОС-процессов | **`runner.py`**; идемпотентность и различение прогонов — **`runId`** (**§6**) |

### 13.2. Планирование для GC

1. **MVP:** не вводить внутреннюю очередь; параллельные прогоны = **несколько независимых вызовов** `graph_caster` (как несколько **`lfx run`**). Обязательно стабильно выдавать **`runId`** в событиях, чтобы консоль и артефакты не смешивали потоки (**E**, **G**).
...
3. **Позже (Aura / SaaS):** если понадобится «как n8n queue» — очередь и воркеры остаются **в хосте**, GC остаётся **чистым исполнителем** (вход: путь/`graphId` + env), выход: NDJSON + артефакты. Не дублировать `ScalingService` внутри репозитория graph-caster.
4. **Внутриграфовый параллелизм** (fork/join ноды): отдельная тема **2b** в плане; **§13** про **межпрогоновый** параллелизм и инфраструктуру очередей.
5. **Мост исполнение ↔ транспорт (фаза 8):** рабочие потоки не должны синхронно звать WebSocket; очередь на event loop — нормальный паттерн (**§13.3**). Глубину буфера и **backpressure** при медленном клиенте закреплять в мосте / **Aura**, не размывать контракт ядра NDJSON — **§39.2**.
...
**Смысл для GC:** при проектировании **моста** (**§39**) полезно явно разделить **(1)** очередь/режим **запуска прогонов** (**F6**, **§13.1**) и **(2)** буфер **нормализованных событий** `run-event` до транспорта; для **(2)** заранее выбрать политику при переполнении (отключение подписки, дроп не-критичных кадров, пауза с таймаутом и т.д.) — **§39.2**.
...
1. **Минимальный путь:** нода **`task`** (обёртка над `curl`/CLI) или отдельный шаг вне графа, вызывающий готовый retrieval API (**Aura**, OpenSearch, внешний Dify) — без embedding-пайплайна в раннере.
2. **Средний путь:** отдельный **микросервис RAG** (единый с **Aura**), GC передаёт `query` + `collection_id` + креды (**§11**); граф остаётся file-first (**F23**).
...
| **GraphCaster** | **`task`:** `retries` + **`process_retry`** / **`process_failed`** | сейчас **стоп** по **`error`** или неуспешному nested run | **план:** либо Aura webhook, либо явная ветка в графе — **16.2** | **`runner.py`**, **`process_exec.py`**, **§3.7** |

### 16.2. Планирование для GC

1. **Не дублировать n8n error-workflow в MVP:** отдельный граф только на ошибку тянет **персистентность workflow id**, права (**§11**), защиту от рекурсии (у n8n проверка `mode === 'error'` и `errorWorkflow === workflowId` в **`executeErrorWorkflow`**). Для file-first **F23** проще: хост (**Aura**, cron) подписывается на **`process_failed`** / финальный **`error`** по **`runId`** (**§12**, **§13**).
2. **Согласовать с Dify-идеей fail branch:** если нужна **альтернативная ветка** без второго файла графа — потребуется поле на ребре или ноде (аналог **`FAIL_BRANCH`**) + правило в **`runner.py`**, не только UI; иначе остаётся «всегда стоп».
...
| **GraphCaster** | Консоль UI по событиям (**§3.7**) | Каталог **`runs/`**, метаданные в **`artifacts`** | **нет** | **`runner.py`**, **`schemas/run-event.schema.json`**, **`ui/src/components/ConsolePanel.tsx`** |

### 17.2. Планирование для GC

1. **Не смешивать фазы:** фаза **7** — фильтры, поиск, экспорт NDJSON из консоли; фаза **8** — надёжная доставка событий (**WebSocket** / буфер); внешний OTel — только при явном требовании **Aura** или enterprise.
...
3. **Тяжёлый путь (как Comfy/n8n):** загружаемые плагины — сразу **модель доверия** (**§11**, песочница, подписи). Для **Aura** разумнее отдельный **реестр доверенных** нод, а не произвольный DLL из интернета.
4. **Связь с F18:** новый **`kind`** почти всегда требует новых **портов** или правил рёбер — обновлять **§15** и **§8** синхронно.
5. **Связь с F10/F7:** RAG- или LLM-нода — не только F15, но и контракт **`task`** / внешнего API (**§14**, **фаза 9**).
...
2. **Фаза 10 + Aura:** выбрать явно — **(a)** optimistic UI + периодический save + разрешение конфликта при `PUT` (версия/ETag), **(b)** CRDT-документ (как **`@n8n/crdt`**) с синхронизацией через хост, **(c)** ветвление в git (два файла графа). Вариант **(b)** не обязан тянуть **зависимость** Yjs **внутрь** `graph-caster`: слой может быть только во фронтенде **Aura**.
...
**F14** у «серверных» конкурентов связывает **идентичность пользователя**, **границу tenant** (workspace / project / organization) и **разрешения** на сущности (workflow, credential, dataset). У **GC** нет user-store: доверие к пути **`graphs/`** задаётся ОС; в **Aura** нужно **воспроизвести только границу доверия**, а не полный ORM **Dify**. **§38** фиксирует, **что остаётся в `graph-caster`**, а что относится только к хосту, без повторения таблицы конкурентов ниже.
...
| **GraphCaster** | **нет в репо** | **нет** | **нет** | Вызывающий процесс (**CLI**, **Aura**) обязан не отдавать чужой **`graphId`** (**§20.2**) |

### 20.2. Планирование для GC

1. **Разделение:** **GC** остаётся «исполнителем и редактором» при валидном входе; **Aura** (или другой хост) решает **кто** может открыть какой **`graphs/...`** или какой blob по **`graphId`**.
...
| **GraphCaster** | **План:** команды над **`GraphDocument`** + view state (viewport) опционально отдельно | **`workspace`/`graphs/*.json`** (**§10**); сброс стека при смене файла — **§21.2** | **Aura** + **F22** может заменить или слить стеки (**§19**) |

### 21.2. Планирование для GC

1. **Инвариант:** каждая отменяемая операция должна иметь чёткий **`redo`** против **`undo`** на уровне **`parseDocument` / `toReactFlow` / `fromReactFlow`** совместимого JSON (**A**), иначе валидатор и UI разъедутся.
2. **Группировка:** перетаскивание ноды во время drag — одна команда при `pointerup`; пакетное удаление — одна команда или макрокоманда — решение в **`PRODUCT_DESIGNE.md`**.
3. **Автосохранение:** после **undo** следующий autosave должен писать **откатанное** состояние; при конфликте с внешним файлом (другой процесс изменил JSON) — либо предупреждение, либо перезагрузка с потерей стека (**как большинство IDE**).
4. **Run-lock (**§7**):** во время активного run разумно **заморозить** структурные команды или копировать граф в «черновик» — аналог изоляции edit/run у **Dify**/n8n; политика UX не в этом документе, но влияет на допустимые transitions стека.
5. **Не тащить Yjs ради undo:** для одного пользователя без **F22** достаточно лёгкого command store (**zustand**/context + массив патчей); **Yjs** — когда появится **§19**.

**Точечные углубления позже:** лимиты глубины стека в **n8n** editor-ui; сравнение **Immer** patches vs явные команды в Dify `web/`; интеграция undo с **F20** и **collaborative** сессией Langflow.
...
| **Vibe Workflow** | — | — | нет |
| **GraphCaster** | план: нода | хэш входов + ревизия документа (**§22.2**) | опционально каталог рядом с **`runs/`** или вложенный в метаданные run |

### 22.2. Планирование для GC

1. **Инвариант:** кэширование имеет смысл только после стабилизации контракта **`data`** ноды и событий **`node_complete`** (**§3.7**); иначе ключи постоянно ломаются.
2. **Два режима (выбрать один в продукте):** (a) **чистый файл-first** — каждый run полный, без скрытого состояния; (b) **явный кэш** — отдельный дисковый или in-memory слой с TTL и лимитом размера (**как Comfy `CacheProvider`**).
3. **Связь с артефактами (**E**): можно переиспользовать сохранённые выходы **прошлого** root-run как «read-only pin» (аналог **`pinData` n8n**), но тогда в UI нужна явная метка «зафиксировано / живое».
4. **Безопасность:** при **Aura** ключ кэша обязан включать tenant / user scope (**§20**), иначе утечка между клиентами.
5. **Не путать:** **`clear_graph_index_cache`** в **`workspace.py`** — только список графов на диске, не **F17**.
6. **Ревизия документа и сброс кэша** (входы нод + версия файла + **`schemaVersion`**) — **§36**; иначе после правки графа возможны «попадания» в старый кэш.
...
| **GraphCaster** | **План:** см. **§23.2** | Через **`task`**, скрипт или внешний HTTP; схема tool — в артефактах run | Минимум: без долговременной памяти в MVP | Расширение **`run-event`** для «tool_start/tool_end» при необходимости (**§3.7**) |

### 23.2. Планирование для GC

1. **Фаза 6 — не путать:** условный переход по полю выхода ноды (**F4**) можно сделать **без** полноценного **F11**; как только нужны **несколько раундов** tool-calling, открывается **§23**.
2. **Минимальный путь:** одна нода **`kind`** (например `llm_agent`), которая **одним** процессом или HTTP запросом делегирует цикл внешнему раннеру; граф GC остаётся **DAG без скрытого внутреннего стека** — проще отладка и **§16**.
...
|-------|---------|----------|-------------|--------------|----------------|
| HTTP webhook | **`Webhook.node.ts`**, регистрация пути на **`WebhookServer`** | **`TriggerWebhookNode`**, HTTP-слой app API | Косвенно через **prediction** / embed; не как отдельная триггер-нода в open core | REST/MCP приём; специализированного «webhook node» мало | Тонкий endpoint **Aura**: auth, **`runId`**, вызов **`graph_caster`** (**§12**) |
...
| Событие / плагин | Триггеры интеграций (Google, …) | **`TriggerEventNode`** (`trigger_plugin`) | Ограниченно | Зависит от деплоя | Очередь сообщений / webhook провайдера → **Aura** |
...
2. **Webhook:** проверка подписи, лимит размера body, **rate limit**, allowlist по IP — на стороне **Aura**; раннер получает уже **проверенный** объект входа (как variable pool у Dify на старте, но без копирования GraphEngine).
3. **Cron:** один вызов = один **root run**; при перекрытии долгого run и следующего tick — политика «пропустить / поставить в очередь / отменить предыдущий» в **§13** или в продукте **Aura**, не в `runner.py`.
...
| **GraphCaster** | **План:** только у **Aura** (JWT, API key, mTLS) | **`POST /v1/runs`** (условно): `graphId`, входы, опции | Редирект на **NDJSON** stream или poll **`runId`** (**§3.7**) | **Не** отдавать полный JSON графа без **§20**; локально — файлы | Минимальная OpenAPI у хоста; ядро — библиотека |
...
4. **Версионирование:** префикс **`/v1`** у Aura; изменения, ломающие клиентов, → новая major-версия; ядро `graph_caster` остаётся **semver библиотеки**, не HTTP.
...
3. **Новые локали:** добавить **`locales/<lng>.json`**, зарегистрировать в **`i18n.init.resources`**; политика **fallback** — **`en`**; RTL только при отдельном ТЗ (**Aura** / enterprise).
...
5. **Встраивание в Aura:** язык пользователя из **JWT/session** может переопределять **`lng`** в **`i18next`** при старте shell; file-first граф (**F23**) по-прежнему без языковых полей в JSON-документе.
...
| **GraphCaster** | **`task`**: **`subprocess.Popen`** + чтение pipe (**stdout**/**stderr**), **`command`** строка или **`argv`**, **`shlex.split`** (POSIX vs Windows), **`cwd`** относительно **`root_run_artifact_dir`**, **`env`** поверх **`os.environ`**, таймаут, **усечённый stdout/stderr**, **`successMode`**, ретраи (**§16**) | Доверие к автору графа и к хосту; секреты только через **§11** | **`process_exec.py`**, ветка **`task`** в **`runner.py`**, схема ноды в **`graph-document.schema.json`** |
...
1. **Фаза 9 (Cursor CLI MVP):** шаблоны **`argv`** и переменные среды из **workspace**/профиля пользователя, без записи ключей в **`graphs/*.json`** — только имена (**§11**); документировать ожидаемый **`exit code`** и **`successMode`** для CLI IDE.
2. **Aura / мульти-тенант:** политика **allowlist** исполняемых файлов или **chroot**/контейнер на воркер — вне **`graph-caster`**, но контракт **`task`** должен позволять хосту **отклонять** команду до **`subprocess`** (**§20**).
...
**F1** у конкурентов почти всегда **не** сводится к библиотеке «ноды+рёбра»: важны **хром ноды** (иконки, статусы, ошибки валидации), **модель ручек** (мульти-выход, типы шин — см. **§15**), **декорации полотна** (комментарии, группы), **навигация** (миникарта, поиск ноды), **жесты** (мультивыбор, выравнивание) и **связь с сервером** (автосохранение vs локальный черновик). У **GraphCaster** осознанный выбор — **@xyflow/react** и единый путь **`ui/src/graph/*`** ↔ **`graph-document`** (**A**).
...
2. **Undo/redo (**F20**):** команды над **`GraphDocument`**, а не только снимок состояния RF — **§21**; автосохранение в **`graphs/`** **батчить**, чтобы не плодить шаги истории на каждый инкремент поля.
...
**Точечные углубления позже:** сравнение **z-index** и hit-testing **n8n** vs RF при 500+ нодах; паттерн **Dify** «подсветка ноды по событию GraphEngine» для GC-консоли (**фаза 8**).
...
| **GraphCaster** | **`graphId`** → путь через **`workspace.py`** | Наследуется **`start`/`exit`** дочернего **A**-документа; рёбра родителя к **`graph_ref`** | Явный **лимит глубины** в раннере; **циклы** между файлами — отлов в **`validate.py`** / при загрузке (**§29.2**) |
...
2. **Валидация:** граф зависимостей по **`graphId`** (цикл A вызывает B вызывает A) — статически, до run, **если** множество файлов доступно индексатору (**C**); при «ленивой» загрузке — детект на глубине с кэшем посещённых id в раннере.
3. **Сбои:** ошибка «файл не найден» / неизвестный **`graphId`** — **`error`** с кодом, не молчаливый skip; согласовать с **§16** (**F19**).
4. **Aura:** список **разрешённых дочерних `graphId`** по tenant (**§20**); не полагаться на file FS в мульти-тенантном деплое без политики **C**.
5. **Асинхронный дочерний run:** не вводить без **§13** (**F6**) и явного продуктового сценария; MVP остаётся **синхронным вложенным** обходом.
...
4. **CI:** регрессионные JSON фикстуры **нескольких** `schemaVersion` в **`python/tests/`** или `examples/`, чтобы раннер не «забывал» старый формат до объявленного EOL.
5. **Aura:** если графы приходят с API, а не только с диска, версия документа должна проходить тот же **`validate`** — один контракт (**§25**, **§20**).
...
| **GraphCaster** | Ожидается **один входной** **`start`** и путь к **`exit`**; **порядок** исходящих рёбер из **A** задаёт перебор при ветвлении (**F4**) | Вложение: синхронный **`graph_ref`** (**§29**) | Данные по связям документа; события **`node_execute`** и др. (**§3.7**) |

**Граница с F4:** **F3** — *какие* ноды считаются достижимыми и в *каком порядке* перечислены рёбра в документе; **F4** — *какое* исходящее ребро выбирается при шаге (**§32**, **`_pick_next_edge`**). У **n8n** часть логики живёт в типах нод (**IF/Switch**), у **GC** — на рёбрах + контекст.

### 31.2. Планирование для GC

1. **Инвариант `start` → `exit`:** для MVP фиксировать, что граф **связен** и существует путь (с учётом **F4** — по крайней мере один исход при каждом ветвлении); **недостижимые** ноды — предупреждение в UI (**`structureWarnings`**) и/или **`validate.py`**.
...
4. **Fork/join (несколько предков):** не притворяться **n8n merge** без явной семантики в **A** и **`runner.py`**; до ТЗ — ограничивать модель или помечать как «вне MVP» (**PRODUCT_DESIGNE.md**).
...
**Точечные углубления позже:** сравнение **Comfy** `get_input_info`/ordering с порядком рёбер GC; политика **n8n** «нода ждёт все входы merge» — при проектировании join-ноды в GC.

---

## 32. Условные рёбра и выбор ветки (**F4**)

**F4** — это **динамический выбор следующего шага** при нескольких исходящих связях из одной ноды. Для **GC** это **не** то же самое, что **топологический порядок** (**§31**, **F3**), **не** цикл model→tool (**§23**, **F11**) и **не** отдельная **ошибочная** ветка движка (**§16**, **F19**), хотя в **Dify** визуально всё соседствует в **GraphEngine**.
...
| **GraphCaster** | Поле **`condition`** на **`Edge`** в **A** | **`_pick_next_edge`**: первое ребро без условия **или** первое, для которого **`_eval_edge_condition`** истинно; иначе **`run_end`** с **`no_outgoing_or_no_matching_condition`** | Литеры `true`/`false`/…; иначе **`bool(context["last_result"])`**; **`node_outputs`** в **D** — расширяемо (**§32.2**) |
...
### 32.2. Планирование для GC

1. **Инвариант «первое подходящее»:** оставить предсказуемое правило: **безусловное** ребро в начале списка «глотает» все последующие до следующего шага — документировать в **`PRODUCT_DESIGNE.md`** и не ломать без **`schemaVersion`**.
2. **Контекст условий:** расширять **не** произвольным **`eval()`**, а ограниченным DSL (**JSONLogic**-подмножество, ссылки на **`node_outputs`**, опционально **`env`** имён) — урок **n8n** (выражения), но с **безопасностью** **file-first** графа (**F23**).
3. **Связь с `task`:** успех/неуспех процесса уже влияет на **`last_result`** (через обновление контекста в **`runner.py`** после шага); при смене правил — обновить **§32** и тесты линейных графов.
4. **Ошибки и альтернативные ветки:** **`FAIL_BRANCH`** / **on_error** (**§16**) — отдельный контракт; не смешивать с «условием на ребре», пока в **A** нет явного типа **ошибочного** исхода.
5. **Fork/join:** несколько **живых** веток с **merge** — **вне** текущей модели (**§31.2**); условные рёбра **не** делают **n8n**-merge сами по себе.
6. **Фаза 6 (AI):** ветвление по **выходу LLM** — либо нода, кладущая структурированный результат в контекст для **§32**, либо внутренний цикл **§23**; не дублировать **агента** полем «промпт в `condition`».
7. **События:** при введении явного **`branch_taken`** / **`branch_skipped`** — **§3.7** + **§8**; до этого **`edge_traverse`** остаётся основным сигналом.
...
2. **Python:** явная политика — **системный** интерпретатор и **`PATH`** vs **sidecar**/встроенный runtime; для **Aura** и CI проще системный; для «один установщик» — отдельный билд-пайплайн (не смешивать с MVP веб-версии).
...
| **GraphCaster** | **Цель:** только **имена** env или opaque **handles**; значения подставляет хост (**Aura**) или ОС при run; **`validate.py`** отсекает неожиданные «секретные» поля после появления схемы | Не коммитить пары ключ/значение API в **`task.env`** в канонических репозиториях |
...
3. **HTTP / MCP (**§25**, **§34**): URL без embedded **basic auth**; ссылка на credential — **id на стороне Aura**, не «логин:пароль» в поле ноды.
...
| **Vibe Workflow** | — | — |
| **GraphCaster** | **Цель:** **`graph_rev`** = хэш канонического JSON (**или** монотонный счётчик при сохранении) + **`schemaVersion`** + хэш релевантных полей **`data`** родителей в графе; опционально **`mtime`** как подсказка, не как единственный ключ | При **autosave** из UI тот же путь файла должен повышать **ревизию**, иначе кэш из прошлого сеанса опасен |
...
2. **Вложенные графы (**§29**):** отдельный **`graphId`** вложения должен входить в ключ кэша вызывающей ноды **`graph_ref`**, иначе смена дочернего файла не инвалидирует родителя.
3. **Aura multi-tenant (**§20**, **§38**):** префикс ключа **`tenant_id`** / **`workspace_id`**; один и тот же **`graphId`** у разных арендаторов — разные кэши.
...
**§16** сравнивает продукты по ретраям, останову и **error workflow**. **§37** задаёт **оси решения** для GraphCaster: что остаётся на **событиях** (**G**, **§3.7**), что требует **нового ребра/поля** в **A**, что выносится в **хост** (**Aura**, **§20**).

### 37.1. Сводка (уровень B)

| Ось | **Dify** | **n8n** | **Flowise** / **Langflow** | Заметка для GC |
|-----|---------|---------|---------------------------|----------------|
| Ошибка как **полный стоп** vs **передача управления** | **abort** vs **FAIL_BRANCH** / **DEFAULT_VALUE** | по умолчанию стоп; **continueOnFail**; отдельный поток error | в основном исключение пробивает run / HTTP 5xx | не смешивать с **`edge.condition`** (**§32**) без явного типа fault-ребра |
...
1. **Базовый уровень:** завершение с **`error`** / **`process_failed`** и стабильным **`runId`**; алерты и ретрай **вне** `runner.py` (**Aura**, **§12**) — без нового **`kind`**.
2. **Ветка в том же файле (Dify-стиль):** отдельное поле ребра или **`kind`** связи (**failure** vs **success**), отличное от строкового **`condition`**; обновить **`runner.py`**, **`branchWarnings`**, **§32**.
...
**§20** сравнивает продукты по tenant, ролям и SSO. **§38** задаёт **контракт для Aura** (или другого хоста): что остаётся в репозитории **`graph-caster`**, а что **не** переносится в Python GC при облачном деплое.

### 38.1. Сводка (уровень B)

| Тема | Внутри `graph-caster` | Только хост (Aura / ОС) |
|------|-------------------------|-------------------------|
...
4. **§34:** схема tools и выполнение шага — в GC; **регистрация** внешнего MCP server в мульти-тенанте — в Aura.
...
| **GraphCaster** | **NDJSON** (stdout / файл) | Плоский **`type`** на корне объекта (**§3.7**) | Нет бинарных кадров в **`run-event`**; артефакты в **`runs/`** (**E**) | WS/SSE — опционально **поверх** тех же событий (**§39.2**) |
...
2. **Адаптер:** одна строка NDJSON ↔ один кадр SSE (`data: …\n\n`) или одно JSON-сообщение в WS; не плодить параллельные enum типов в мосте. Не полагаться на «**`text/event-stream`** = уже валидный **EventSource**» без проверки тела (**§3.4.1** — контрпример).
...
6. **Backpressure:** мост не должен накапливать **неограниченно** кадры при отстающем потребителе. Эталон проблемы — **`asyncio.Queue()`** без `maxsize` у **`PromptServer.messages`** (**§13.3**): быстрый продюсер событий, медленный клиент. Для GC / **Aura** зафиксировать политику: ограниченная глубина, отключение подписки, дроп некритичных типов или иной явный контракт; блокировать **`runner`/процесс `task`** из-за медленного UI — только осознанно (влияет на **latence** прогона).
...
8. **Удалённый раннер без push в BFF:** если единственный контракт — **HTTP** start + **poll** статуса (как открытый контур **Vibe** — **§3.8** / **§3.8.1**), UI **Aura** не получает частый **`run-event`** без доп. канала; либо осознанно принять грубый **poll**, либо договориться с хостом о **SSE/WS** по схеме **§39.2** п.2.

**Точечные углубления позже:** сравнение с bounded queues в других стэках; лимиты прокси на длину SSE-чанка; политика при `QueueFull` в Python-мосте; полный **v2 workflow** stream в backend Langflow vs **LFX** — при сближении с продуктовым Langflow HTTP.
