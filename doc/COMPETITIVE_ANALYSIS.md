...
**Дата:** 2026-03-29 · **итерация 72** (**Follow run** / **`FollowActiveRunCamera`** — факты в [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md), строка **«Опционально следить камерой…»**; выполненный план удалён **`doc/PLAN_follow_active_node_during_run.md`**; **MiniMap** — маска вне вьюпорта и рамка вьюпорта (**`minimapChrome.ts`**, **`usePrefersColorSchemeDark`**) — раздел **«MiniMap и панель управления полотном»** в том же файле; черновик плана удалён **`doc/plans/2026-03-29-minimap-viewport-chrome.md`**; **F18** жест connect — **`isValidConnection`**, подсветка целевых ручек — в том же [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (таблица **Canvas / F1**, строка **«F18 / Langflow+n8n+RF…»**, раздел **«Статическая совместимость ручек F18»**); черновик плана удалён **`doc/plans/2026-03-29-f18-connection-target-highlight-and-isvalid.md`**; **`doc/plans/`** — временные черновики, каталог в **`.gitignore`**; **`mergeGraphEdgeData`** и полный перечень F18 — раздел **«Статическая совместимость ручек F18»** в [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md); **Steps / execution timeline** (**`buildRunTimeline.ts`**, **`reduceConsoleLinesToRunTimeline`**, вкладка **Steps** в **`ConsolePanel`**) — раздел **«Консоль наблюдаемости»** (**F13**) в том же файле; черновик плана удалён **`doc/plans/2026-03-29-run-execution-timeline-panel.md`**; **§29** / **§32.2** / **§39** / оверлей после воркера — отсылки там же; **MCP (A)** **`graphcaster_cancel_run`** — кооперативная отмена по **`run_id`** в процессе stdio ([`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) — строка **«Отмена прогона из MCP-процесса»**); черновик плана удалён **`doc/plans/2026-03-29-mcp-cancel-run-tool.md`**; **SQLite-каталог прогонов** — раздел **«Каталог прогонов (SQLite)»** там же; выполненные планы **`doc/plans/2026-03-29-run-execution-catalog-sqlite.md`**, **`doc/plans/2026-03-29-workspace-execution-history-ui.md`** удалены)  
...
|------|-----------------|------------------------|
| **A. Документ и схема** | JSON Schema, `GraphDocument`, версии, инварианты графа; контракт строк раннера | `schemas/graph-document.schema.json`, **`schemas/run-event.schema.json`**, `models.py`, `validate.py`; **`schemaVersion`** и миграции документа — **§30** (**F2**); **F18** (ручки + **`PortDataKind`**, в т.ч. переопределения на ребре) — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md); хвост типизации (**`blob_ref`**, мультишины, медиа-сокеты) — **§15**; новые **`kind`** нод — **§18** (**F15**) |
| **B. Редактор (UI)** | Канвас, ноды, инспектор, предупреждения, экспорт, история правок | `ui/` (**@xyflow/react**), `ui/src/graph/` (в т.ч. **`nodePalette.ts`**, **`nodeKinds.ts`**); канвас и паттерны UX — **§28** (**F1**); **палитра поиска**, **ПКМ → добавить ноду** (чипы категорий), **sidebar node palette** с **drag-and-drop** (**n8n / Langflow style**), **мультивыбор / буфер / групповое удаление**, защита **`start`**, **большие графы** (viewport/оверлей/sync рёбер), **рамки `group` / Group selection** — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (разделы «Поиск…», меню добавления ноды, **«Sidebar Node Palette with Drag & Drop»**, «Мультивыбор…», **«Canvas: большие графы»**, **«Визуализация прогона на канвасе»**); **нативная оболочка** (**Tauri**) — **§33** (**F16**); **Run/Stop** из десктопа — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md); **веб Run/Stop** (`npm run dev:web` + **`serve`**, SSE или WebSocket брокер) — там же, **«Веб без Tauri»** и **«Dev WebSocket и `run_transport`»**; **частичный прогон** (инспектор → **`untilNodeId`**) — там же, раздел «Частичный прогон»; инспектор + маркер на канвасе для **`gcPin`** (**`task`**, аналог n8n **`pinData`**) — там же, «Закреплённый вывод…»; **`task`** / **`mcp_tool`** / **`ai_route`** / **`llm_agent`**: **`data.stepCache`**, бейдж **C**, кнопка **dirty** и тоггл **Step cache** в панели Run (F17) — там же, «Межпрогонный кэш…»; предупреждения по ручкам (**F18**) — там же; эталоны пинов у конкурентов и открытые темы — **§15**; типы нод — **§18**; i18n — **§26** (**F21**); undo/redo (**F20**) — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md), детали — **§21**; **CI** (Vitest/`npm run build` + pytest в родительском репо) — там же |
...
| **G. Наблюдаемость** | Логи исполнения, история, трейсинг, экспорт | Консоль UI + NDJSON (**§3.7**), артефакты **`runs/`**; стрим, жизненный цикл **`runId`**, **UX консоли** (фильтры, поиск, экспорт, переход к ноде; вкладка **Steps** — список шагов из того же буфера NDJSON — фаза 7); **оверлей прогона на канвасе** (фазы нод — **`nodeRunOverlay`**; последнее пройденное ребро — **`runEdgeOverlay`**; live, replay и **settled** после процесса — **`GraphCanvas`** / **`runSessionStore`**; опц. **Follow run** — **`FollowActiveRunCamera`**) — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (**F13** + **«Визуализация прогона на канвасе»**); **offline-список прошлых прогонов + replay** (файловый журнал; проверка **`rootGraphId`** при replay — там же); **деривативный SQLite-индекс** метаданных прогонов (кросс-графовый list/filter, rebuild, **`GC_RUN_CATALOG`**) — там же (**«Каталог прогонов (SQLite)»**); **bounded queue** dev run-брокера (**SSE** / **WebSocket**, **`stream_backpressure`**) — там же (**«Backpressure SSE»** + **Evidence**); **нормализация финала при обрыве воркера** (synthetic **`run_finished`**) — там же, строка **«Обрыв воркера»** в **«Десктоп…»**; **§17** — сравнение с конкурентами; **остаточный** **F13**: ORM-уровень **`Execution`** как у Flowise/n8n, **prod**-транспорт **§39** / буфер **§39.2** без dev-брокера; опциональный OTLP-трейсинг раннера — факты в [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (**F13**). Без дубля фактов UI |
...
| **GraphCaster** | **частично** | Слой **B** (**полотно, инспектор, предупреждения**, **@xyflow/react**, ноды **`GcFlowNode`** / **`GcCommentNode`** / **`GcGroupNode`**): закрытые пункты **F1** / **§28.2** (канвас, **MiniMap** — обзор и **маска/рамка вьюпорта**, поиск, clipboard, **group**, **snap/align**, **LOD**, **off-viewport ghost**, **превью связи** от ручки, **ленивое превью `graph_ref`**, **F13**, открытие/Save, **sidebar node palette** с **drag-and-drop** n8n/Langflow-style …) — **единый SSOT** [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (**«Canvas: большие графы»**, **«Sidebar Node Palette with Drag & Drop»** и смежные разделы, **в т.ч. подписи на рёбрах** F4 / **`ai_route`** — **§28.2** п.8, превью линии — п.9); здесь перечень не дублировать. **Остаток F1:** мультишины и прочее — **§15.2**. Типизация портов (**`PortDataKind`**, в т.ч. **`edges[].data.sourcePortKind` / `targetPortKind`**) — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (**F18**). **§29** (встраивание дочернего графа в **A** вне file-first). |
...
| **GraphCaster** | **частично** | **G+E:** **`run-event.schema.json`**, папки артефактов, **`ConsolePanel`**, стрим (**`gc-run-event`**); **файловый журнал** (**`events.ndjson`**, **`run-summary.json`**, **History**); **оверлей статусов нод на канвасе** (live и replay из журнала, **`nodeRunOverlay.ts`** / **`GraphCanvas`**) — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md); **опционально OpenTelemetry** OTLP в раннере (**`graph-caster[otel]`**, spans **`gc.run`** / **`gc.node`**) — там же (**F13**, OTLP); **нет** облачной ORM-БД **`Execution`** как у Flowise/n8n — **§17.2**. |
...
| **GraphCaster** | **частично** | **A+B:** именованные ручки **`in_default`/`out_default`/`out_error`**; контракт по **`node.type`** + **`PortDataKind`** и опциональные переопределения на ребре (**`edges[].data`** — см. **F18** в [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md)); жёсткая **`validate_graph_structure`** только на неверных id ручек. **Остаток к эталонам:** **`blob_ref`**, расширение видов данных, мультишины **n8n** — **§15.2**. |
...
**F18** связывает **редактор** (что можно соединить на canvas), **схему документа** (какие поля ребра/портов в JSON) и **рантайм** (что исполнитель готов читать). У конкурентов это либо **жёсткая типизация сокетов** (Comfy), либо **несколько семейств рёбер** (n8n), либо **Pydantic + отдельный граф-валидатор** (Dify Graphon). **Уже в GraphCaster:** статический контракт handle, **`PortDataKind`** (фаза 1 таблица + **фаза 2** опциональные **`edges[].data.sourcePortKind` / `targetPortKind`**) до run, плюс **во время жеста** connect — отказ несовместимого drop и подсветка целевых ручек (**`isValidConnection`**, **`connectionCompatibility.ts`**) — единый SSOT фактов: [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (**раздел «Статическая совместимость ручек F18»** и строка **Canvas / F1** про **F18 / Langflow+n8n+RF**); ниже — сравнение с продуктами и **открытый** хвост (**§15.2** только про расширение типов / мультишины, не про базовый UX connect).
...
| **GraphCaster** | **F18:** контракт handle **`node.type`** + **`PortDataKind`** (таблица + опциональные переопределения на ребре в **`edges[].data`**); предупреждения kind в UI и **`structure_warning`** в раннере (без **`GraphStructureError`** на kind); **живой connect** — отказ drop и подсказки на ручках (предикат **`connectionCompatibility.ts`**) | UI: **`handleContract.ts`**, **`portDataKinds.ts`**, **`portDataKindCompat.ts`**, **`handleCompatibility.ts`**, **`connectionCompatibility.ts`**, **`GraphCanvas.tsx`** (**`isValidConnection`**), **`GcFlowTargetHandle.tsx`**, **`GcConnectionDragContext.tsx`**, **`structureWarnings.ts`**, **`branchWarnings.ts`**; Python: **`handle_contract.py`**, **`port_data_kinds.py`**, **`validate.py`**; схема: **`graph-document.schema.json`** `$defs.edgeData`; слияние патчей **`edges[].data`**: **`mergeGraphEdgeData.ts`** (см. SSOT) | Факты реализации — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (**«Статическая совместимость ручек F18»** + **Canvas / F1** для connect). Невыполненный хвост типизации — **§15.2** ниже. |

### 15.2. Планирование для GC

**Закрыто в продукте** (имена ручек, **`PortDataKind`** фаза 1–2, в т.ч. **`edges[].data.sourcePortKind` / `targetPortKind`**, пути к коду и тестам) — единый SSOT: [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md), раздел **«Статическая совместимость ручек F18»**. Ниже — **только** невыполненный хвост и идеи.

1. **Документ и расширение типов:** **`blob_ref`**; новые виды сверх **`any`/`json`/`primitive`**; мультишины (**n8n**); **не копировать Comfy 1:1** (IMAGE/LATENT, …) без домена медиа. Новый **`kind`** ноды задаёт набор портов — **§18** (**F15**).
2. **n8n как идея мультишин:** если появятся **LLM-цепочки** рядом с **обычным** `task`, можно разнести роли рёбер (аналог `main` vs `ai_*`) без смешения в одном handle; иначе достаточно типизированных имён портов на одном ребре.
3. **Связь с F19:** ошибка **исполнения** шага — **`process_*`** / **`out_error`**; инварианты **совместимости ручек** — до run (**F18**); расширенная политика «несовместимое ребро при сохранении» — по продукту, **§16** / **F19**.
...
**Реализованный слой:** консоль (**F13**, фаза 7), в т.ч. вкладка **Steps** / **`reduceConsoleLinesToRunTimeline`** (**`buildRunTimeline.ts`**) — список шагов как у n8n execution list без второго транспорта; **оверлей исполнения на канвасе** (статусы нод по **`run-event`**, live, replay и **settled** после выхода воркера, per **`rootGraphId`**) — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (**F13**); **файловый журнал прогона** (**`events.ndjson`**, **`run-summary.json`**, UI **History**, брокер/Tauri) — там же; **деривативный SQLite-индекс** метаданных (**`runs_catalog.sqlite3`**, list/rebuild, **`run_catalog`**) — там же (**«Каталог прогонов (SQLite)»**); **dev run-брокер** (`serve`, **SSE** / **WebSocket**) и bounded backpressure — там же (**«Backpressure SSE»** + **Evidence**), без дубля в этом разделе. Ниже — **сравнение с конкурентами** и темы **F13**, которые в **`graph-caster`** остаются **вне** этого среза (**OTel**, полный ORM-**`Execution`** как у SaaS, **prod**-транспорт **§39** / relay **§39.2** п.7 и п.6 — политика хоста).

**F13** у конкурентов почти всегда многослойна: **(1)** поток событий «что сейчас выполняется» для UI, **(2)** сохранённый **execution** (аудит, отладка, биллинг), **(3)** опционально **распределённый трейсинг** (OpenTelemetry) и **LLM-специфичные** экспорты (Langfuse, LangSmith, Phoenix). У **GC** сильны **(1)** в виде плоских NDJSON-событий; **(2)** закрыт **file-first** срезом (NDJSON + сводка в каталоге рана) и **опциональным** локальным **SQLite sidecar** только для метаданных (не тело прогона) — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (**«Каталог прогонов (SQLite)»**); полный паритет SaaS-**`Execution`** / облачная БД в core — нет; **(3)** не внедрялся.
...
| **GraphCaster** | Консоль по событиям (**§3.7**), в т.ч. инкрементальный вывод **`task`** (**`process_output`** в NDJSON); вкладка **Steps** — **`reduceConsoleLinesToRunTimeline`** (**`buildRunTimeline.ts`**) из NDJSON (ориентир n8n execution list); **оверлей статусов нод на канвасе** (live / replay / **settled** после процесса, **`nodeRunOverlay.ts`**, **`runSessionStore.ts`**, **`GraphCanvas`**, i18n **`app.run.overlay.*`**, **`app.run.clearSettledVisual*`**) — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (**F13**) | **File-first:** **`events.ndjson`** + **`run-summary.json`** при **`--artifacts-base`**; список/чтение через Tauri и **`POST /persisted-runs/*`**; **деривативный индекс:** **`.graphcaster/runs_catalog.sqlite3`**, **`POST /run-catalog/*`**, **`catalog-rebuild`** — раздел **«Каталог прогонов (SQLite)»** там же; UI **History** + replay с тем же оверлеем; предупреждение при несовпадении **`rootGraphId`** лога и открытого графа — там же | **нет** | **`run_event_sink.py`**, **`artifacts.py`**, **`runner.py`**, **`run_catalog.py`**, **`process_exec.py`**, **`run_broker/`**, **`run_bridge.rs`**, **`RunHistoryModal`**, **`schemas/run-event.schema.json`**, **`ConsolePanel`**, **`buildRunTimeline.ts`**, **`parseRunEventLine.ts`**, **`runEventSideEffects.ts`**, **`runSessionStore.ts`** |

### 17.2. Планирование для GC

1. **Не смешивать фазы:** фаза **7** — фильтры, поиск, экспорт из консоли, привязка к ноде, вкладка **Steps** (список шагов из NDJSON — **`buildRunTimeline.ts`**) (**сделано** — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md)); **оверлей статусов нод на канвасе** по потоку **`run-event`** (live / replay / **settled** после выхода воркера — см. строку **«Итог прогона на холсте…»** в **«Визуализация прогона на канвасе»** того же файла) — **сделано**, там же (**F13**); фаза **8** — надёжная доставка событий (**WebSocket** / буфер) и мост UI↔Python; внешний OTel — только при явном требовании **хост** или enterprise.
...
3. **Артефакты vs «база execution»:** file-first **F23** + **локальный** деривативный индекс в SQLite реализованы в **`graph-caster`** — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (**«Каталог прогонов (SQLite)»**); облачный мульти-тенант ORM и обязательный **`Execution`** Flowise в core по-прежнему не требуются. Хост может добавить отдельный индекс/БД поверх тех же артефактов.
...
8. **Подписи на рёбрах** (условия **F4**, описания веток **`ai_route`**, переключатель в шапке, **LOD**): **закрыто** — строка таблицы **«Подписи на рёбрах…»** в том же разделе **«Canvas: большие графы»** [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md); детали реализации в **§28** не дублировать.
9. **Превью линии соединения** при перетаскивании от **Handle** (радиус подсадки **`connectionRadius`**, **Bezier** согласованно с **`GcBranchEdge`**, контрастный stroke light/dark): **закрыто** — строка таблицы **«n8n / Langflow… протягивание связи»** в разделе **«Canvas: большие графы»** [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md); в **§28** не дублировать.
10. **Co-edit (**F22**):** не синхронизировать сырой internal-state **React Flow** по сети — только **документ** (**§19**).
...
5. **Конкурентность:** один MCP session vs несколько параллельных tool calls — политика как у **§13** на уровне хоста (очередь воркера); в текущем stdio MVP — см. ограничения в **`python/README.md`** (**`workerStillRunning`**, таймаут + cooperative cancel, tool **`graphcaster_cancel_run`** по **`run_id`** — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md), без повторения эталонов конкурентов здесь).
