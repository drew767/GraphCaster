...
| **GraphCaster** | Ожидается **один входной** **`start`** и путь к **`exit`**; **порядок** исходящих рёбер из **A** задаёт перебор при ветвлении (**F4**). **Статический** список нод вне обхода от **`start`** (все рёбра считаются проходимыми) — см. [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (**F3**). | Вложение: синхронный **`graph_ref`** (**§29**) | Данные по связям документа; события **`node_execute`** и др. (**§3.7**) |
