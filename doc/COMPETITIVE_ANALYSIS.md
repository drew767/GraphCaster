...
**Дата:** 2026-03-29 · **итерация 65** (**§32.2**: узкий **`$node`** в шаблонах и **`var`**; «открыто» — полный **n8n Expression** / JS sandbox)  
...
**Уже реализовано и вынесено из «пробела» относительно конкурентов:** см. [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (host/run state, жизненный цикл NDJSON с `runId`, **десктоп:** мост UI ↔ Python через Tauri subprocess; **веб (dev):** локальный run-брокер **`python -m graph_caster serve`**, Vite прокси **`/gc-run-broker`**, **SSE** на **`runId`** (тот же контракт событий, что у CLI) — раздел **«Веб без Tauri»**; **частичный прогон** — **`--until-node`**, **`run_finished.status` `partial`**, **`--context-json`** — раздел «Частичный прогон»; **инкрементальный вывод подпроцесса `task`** (**NDJSON `process_output`**, живые логи в консоли) — раздел с тем же названием в том же файле; **dev SSE backpressure** (**`RunBroadcaster`**, **`stream_backpressure`**, ограниченные очереди подписчиков, без глобального лока на `broadcast`) — подраздел **«Backpressure SSE»**; **закреплённый вывод `gcPin`** (UI + раннер, аналог n8n **`pinData`** для **`task`**) — подраздел «Закреплённый вывод…»; **межпрогонный кэш `task`** — **`--step-cache`**, **`--step-cache-dirty`**, **`node_cache_*`**, в десктопе — тоггл Run, очередь **dirty**, **транзитивное замыкание** в духе n8n **`dirtyNodeNames`** (успешные рёбра, активный документ) — подраздел «Межпрогонный кэш выходов **task**»; **вложенный `graph_ref`** + step-cache **dirty** (стек навигации, **bubble** с диска, **`_parent_graph_ref_node_id`** в раннере; паритет sub-workflow / subgraph у конкурентов) — отдельный абзац **«Вложенный `graph_ref`»** перед той же таблицей F17; **нода `ai_route`** (статическое ИИ-ветвление исходов, wire v1, **`choiceIndex`**, события **`ai_route_*`**) — подраздел **«ИИ-ветвление / нода `ai_route`»** в разделе **F4** того же файла; **файловый журнал прогонов** (**`events.ndjson`**, **`run-summary.json`**, UI **History**, replay в консоль, брокер **`POST /persisted-runs/*`**) — раздел **«Персистентный журнал прогона / execution history»** в том же файле; **ошибки при открытии графа** (чтение / синтаксис JSON / **`parseGraphDocumentJsonResult`**, модалка **`OpenGraphErrorModal`**, i18n, имя файла в заголовке — P1 в **`DEVELOPMENT_PLAN.md`**) — раздел **«Открытие графа: ошибки JSON и парсера»**; **CI** монорепо **Aura** для pytest + UI build — отдельный подраздел там же).
...
| **B. Редактор (UI)** | Канвас, ноды, инспектор, предупреждения, экспорт, история правок | `ui/` (**@xyflow/react**), `ui/src/graph/` (в т.ч. **`nodePalette.ts`**, **`nodeKinds.ts`**); канвас и паттерны UX — **§28** (**F1**); **палитра поиска**, **мультивыбор / буфер / групповое удаление**, защита **`start`** на полотне — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (разделы «Поиск…», «Мультивыбор…»); **нативная оболочка** (**Tauri**) — **§33** (**F16**); **Run/Stop** из десктопа — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md); **веб Run/Stop** (`npm run dev:web` + **`serve`**, SSE-брокер) — там же, **«Веб без Tauri»**; **частичный прогон** (инспектор → **`untilNodeId`**) — там же, раздел «Частичный прогон»; инспектор + маркер на канвасе для **`gcPin`** (**`task`**, аналог n8n **`pinData`**) — там же, «Закреплённый вывод…»; **`task.data.stepCache`**, бейдж **C**, кнопка **dirty** и тоггл **Step cache** в панели Run (F17) — там же, «Межпрогонный кэш…»; предупреждения по ручкам (**F18**) — там же; эталоны пинов у конкурентов и открытые темы — **§15**; типы нод — **§18**; i18n — **§26** (**F21**); undo/redo (**F20**) — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md), детали — **§21**; **CI** (Vitest/`npm run build` + pytest в родительском репо) — там же |
...
| **F. Интеграции** | Credentials, HTTP, внешние API, RAG / knowledge, вызовы LLM/tools, внешний старт прогона, публичный контур | HTTP к внешнему провайдеру для ветвления — нода **`ai_route`** ([`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md)); сейчас основной фокус на **`task`**/процесс; креды — **§11** / **§35** (**F8**); триггеры — **§24** (**F9**) + **§12**; **REST/embed/OpenAPI** — **§25** (**F12**), BFF-эталон — **§25.3** (Vibe); **MCP** (граф как server tool / вызов внешних tools) — **§34**; RAG — **§14** / **F10**; полноценные агенты — **§23** (**F11**) |
| **G. Наблюдаемость** | Логи исполнения, история, трейсинг, экспорт | Консоль UI + NDJSON (**§3.7**), артефакты **`runs/`**; стрим, жизненный цикл **`runId`**, **UX консоли** (фильтры, поиск, экспорт, переход к ноде — фаза 7); **offline-список прошлых прогонов + replay** (файловый журнал); **bounded queue** dev SSE-брокера (**`stream_backpressure`**) — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (строка таблицы «Backpressure SSE»); **§17** — сравнение с конкурентами и **остаточный** **F13** (OTel, ORM-уровень **`Execution`** как у Flowise/n8n, **prod**-транспорт **§39** / буфер **§39.2** без dev-брокера), без дубля фактов UI |
...
**Для GC:** не копировать объём **`ExecutionPushMessage`** и политику redaction; в n8n «живой» прогон — **отдельный канал** с **`pushRef`**, не тот же REST. **Строка NDJSON с одним `runId` на корневой прогон** + границы **`run_started` / `run_finished`** + статус **`cancelled`** — **сделано** в Python ([`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md); **§3.7**). **Отдельно от WebSocket:** in-process **отмена** и **stdin**-команды — см. раздел **«Реестр корневых прогонов и отмена»** в том же файле. **Веб (разработка):** локальный **HTTP + SSE** (`text/event-stream`) на **`/runs/{runId}/stream`**, спавн того же CLI `run`, что и Tauri — раздел **«Веб без Tauri»** там же (без типового n8n **`ExecutionPushMessage`**, без общего **`/push`** и prod-auth). **Ещё не сделано:** паритет n8n — единый **WebSocket** или прод-**SSE** с сессионным **`pushRef`**, типами **`ExecutionPushMessage`**, relay при вынесенном воркере — **§39** (ср. Flowise **§3.3.1**, **§39.2** п.7).
...
| **GraphCaster** | **частично** | Слой **B**: **@xyflow/react**, **`GcFlowNode`** / **`GcCommentNode`**, **`normalizeHandles`**, инспектор, предупреждения структуры; факты реализации (**F18**, **undo/redo**, **run-lock**, **P2** sanitize, **MiniMap/Controls**, поиск на полотне, мультивыбор/clipboard/групповое удаление, защита **`start`**) — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md); **§28.2** — **открыто** только производительность очень больших графов (**виртуализация** / **lazy**-подграфы после профилирования); **CI** — там же; полная типизация пинов — **§15**. |
...
| **GraphCaster** | **частично** | Слой **D**: обход **`start`**→**`exit`**, **F4** (**§32**), вложение **`graph_ref`** (**§29**) — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md). **`merge`** с **`data.mode`** **`barrier`** и нода **`fork`** (fan-out без OS-потоков) закрывают срез **barrier-join / fork** относительно **§31.2**; циклы и полная рантайм-«связность» с симуляцией **F4** — по-прежнему см. competitive. |
...
| **GraphCaster** | **частично** | **D+B**, **§32**: рантайм ветвления, DSL условий (**`$json`**, **`$node`**, шаблоны, JSON Logic) — реестр и пути в коде в [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (**F4**); UI — **`branchWarnings`**. **Без** полноценного **n8n Expression** (JS sandbox). In-graph **`out_error`** — там же (**F19**). Нет паритета с полным expression runtime **n8n**, отдельным **error-workflow** как у **n8n**, полным **F11** — `DEVELOPMENT_PLAN.md` / **§23**. |
...
| **GraphCaster** | **частично** | Слой **D+C**: **`kind`** **`graph_ref`**, **`graphId`**, загрузка из **`workspace.py`**, синхронный вложенный прогон; события nested; лимит глубины; **циклы между файлами по `graphId`** — **закрыто:** [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (**F5**); прочее — **§29.2** (Aura, async, политика сбоев). |
...
| **GraphCaster** | **частично** | **Статическое** ИИ-ветвление исходов — нода **`ai_route`** (wire v1, внешний HTTP-провайдер **`choiceIndex`**) — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) («ИИ-ветвление / нода **`ai_route`**»). Полноценный цикл **F11** (ReAct, память, многораундовый tool-calling) — **план**, **§23.2** (внешний orchestrator или отдельные **`kind`**, не дублировать **Dify**/**n8n** в ядре). |
...
| **GraphCaster** | **частично** | **G+E:** **`run-event.schema.json`**, папки артефактов, **`ConsolePanel`**, стрим (**`gc-run-event`**); **файловый журнал** (**`events.ndjson`**, **`run-summary.json`**, **History**) — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md); нет OTel и облачной БД **`Execution`** — **§17.2**. |
...
| **GraphCaster** | **частично** | **Факты в коде** — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md): «Частичный прогон», «Межпрогонный кэш выходов **`task`**», «Закреплённый вывод (**`gcPin`**)», транзитивная очередь **dirty** (успешные рёбра). **Зазор к n8n Comfy-grade F17:** тоггл step-cache на полотне, кэш произвольных нод (не только **`task`**), TTL/лимиты — **§22.2**, **§36**. |
...
2. **Не брать в MVP** без явного решения: **полный** **F6** (очередь **многих** прогонов, Redis/n8n queue mode, worker pool, **параллельное** исполнение веток внутри одного прогона) — **§13**; **внутренний** синхронный срез (sink + **StepQueue**, последовательный **`fork`** / **`merge`** **`barrier`**) — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md). **F10** (ingestion + vector DB + раннер) — **§14**; **F14** (tenant, RBAC, SSO) — **§20** / **§38**, не дублировать Dify/n8n в `graph-caster`. **F8** — **§11** / **§35**. **F9**/**F12** — **§12**; триггеры — **§24**; публичный REST, ключи, OpenAPI — **§25**. **F22** (реалтайм co-edit, Yjs) — **§19**; не смешивать с **фазой 5** run-lock (локальная блокировка UX).
...
5. **Фаза 8 (мост):** у **Dify** в монолите буфер между GraphEngine и HTTP — **in-process** **`queue.Queue`** (**§3.6.1**), чтение в **`StreamResponse`** и **`break`** по финальным **`Queue*`** — **§3.6.2**; у **Flowise** в queue mode — **Redis** (**§3.3.1**). **У GC** стабильный **`runId`** на строках NDJSON корневого прогона (аналог `prompt_id` / **`workflow_run_id`**) и маршрутизация в UI — **закрыто:** [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (раздел «Стабильный `runId`…»), **`schemas/run-event.schema.json`**, **§3.7.1**; путаница нескольких сессий в UI — очередь/cap и фокус там же. Внутренний маппинг «раннер → консоль» можно держать плоским, отдельный слой Queue* (**§3.6**) — только если появятся разные потребители одного прогона. Актуальный перечень `type` и пробелы — **§3.7**. Разделение **очереди исполнения** и **буфера к транспорту** (эталон Comfy — **§13.3**); при вынесении воркера — мост событий (эталон Flowise **Redis** — **§3.3.1**); явная политика при медленном клиенте — **§39.2** (dev **SSE** — факты в [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) **«Backpressure SSE»**).
...
20. **Вложенные графы (**F5**):** **`graph_ref`** только на существующий **`graphId`** (**C**, **§10**); **циклы по workspace** — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (**F5**); **глубина вложенности** — раннер (**§29**); **`runId`** корневого прогона не «терять» во вложенных событиях (**§3.7**, **§17**); в Aura — ACL «какой граф можно вызывать» (**§20**, **§38**).
...
**Дальнейшие точечные углубления:** Comfy — инвентаризация **`send_sync`/JSON `type`** по **конкретной** установке (diff форка vs upstream; методика — **§3.5.1**); **Flowise** — выравнивание **`RedisEventPublisher.streamMetadataEvent`** с **`SSEStreamer`** (или осознанная политика урезания) — поверх **§3.3.3**; Dify — OpenAPI / сопоставление маршрутов с веткой **`InvokeFrom`** (**§3.6.3**); n8n — при смене версии субмодуля повторить **`sensitiveOutputFields`** по **`packages/nodes-base`** и при необходимости по закрытым пакетам билда (**§3.2.5** — снимок open tree); Vibe — тела ответов **run/status** MuAPI (вне open repo); каталог прокси **BFF → MuAPI** — **§3.8.1**; **`runId`** в потоке GC — зафиксировано в **`schemas/run-event.schema.json`**, факты — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) и **§3.7.1**; публичная схема **Queue*** в OpenAPI Dify — только если интегрироваться с Dify, а не с собственным раннером GC.
...
1. **MVP:** **межпрогоновый** параллелизм = **несколько независимых вызовов** `graph_caster` (как несколько **`lfx run`**). **Внутри одного прогона** — FIFO **шагов** + **`RunEventSink`** к stdout — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (**F6** срез); не путать с очередью **многих** прогонов / Redis (**n8n queue**). **`runId`** на событиях обязателен (**E**, **G**).
...
4. **Внутриграфовый OS-параллелизм** веток (одновременное исполнение после fan-out): **§13** / **F6**; последовательный **`fork`** + **`merge`** **`barrier`** — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md). Далее в **§13** — **межпрогоновый** параллелизм и инфраструктура очередей.
5. **Мост исполнение ↔ транспорт (фаза 8):** рабочие потоки не должны синхронно звать WebSocket; очередь на event loop — нормальный паттерн (**§13.3**). Глубину буфера и **backpressure** при медленном клиенте закреплять в мосте / **Aura**, не размывать контракт ядра NDJSON — **§39.2** (dev **SSE** — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) **«Backpressure SSE»**).
...
**Реализованный слой:** консоль (**F13**, фаза 7) и **файловый журнал прогона** (**`events.ndjson`**, **`run-summary.json`**, UI **History**, брокер/Tauri) — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md); **dev run-брокер** (`serve`, SSE), **§39.2** п.6 (bounded backpressure) — только там (**«Backpressure SSE»**), без дубля в этом разделе. Ниже — **сравнение с конкурентами** и темы **F13**, которые в **`graph-caster`** остаются **вне** этого среза (**OTel**, ORM-**`Execution`**, **prod**-транспорт **§39** / relay **§39.2** п.7).
...
1. **Не смешивать фазы:** фаза **7** — фильтры, поиск, экспорт из консоли, привязка к ноде (**сделано** — `IMPLEMENTED_FEATURES.md`); фаза **8** — надёжная доставка событий (**WebSocket** / буфер); внешний OTel — только при явном требовании **Aura** или enterprise.
...
| **GraphCaster** | **частично (MVP):** снимки **`GraphDocument`**, см. [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) — не отдельные **apply**/**revert** на шаг как у **Dify**; **viewport** в стеке — нет | **`workspace`/`graphs/*.json`** (**§10**); сброс стека при смене файла — как в реестре фич | **Aura** + **F22** может заменить или слить стеки (**§19**) |
...
| **Vibe Workflow** | — | — | нет |
| **GraphCaster** | **частично:** **`task`** + диск + **`gcPin`** + **десктоп:** тоггл **Step cache**, очередь **dirty** (транзитивно по успешным рёбрам → **`--step-cache-dirty`**, как **`dirtyNodeNames`** у n8n) + **bubble** при правках во вложенном **`graph_ref`** (стек навигации, **`_parent_graph_ref_node_id`** в раннере) — см. [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md); **полный** F17 — **§22.2** | Ключ: ревизия JSON + отпечаток предков + **`data`** ноды (см. [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md)) | **partial**; **`gcPin`** + межпрогонный кэш **`task`** под **`artifacts_base`** — **да** (CLI и Tauri); кэш не-**`task`** / TTL — **нет** (**§22.2**) |
...
3. **`§22.2` — что ещё не закрыто относительно «полного» n8n/Comfy:** pin для всех типов исполняемых нод (не только **`task`**); отдельный продуктовый тоггл «зафиксировано / живое» не только через **`gcPin.enabled`**. Транзитив **`dirty`** при правках во вложенном **`graph_ref`** — **реализовано**, канон описания только в [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (абзац **«Вложенный `graph_ref`»** перед таблицей F17). **Связь с артефактами (E):** **`--context-json`**, **`gcPin`**, **`node_outputs_snapshot`** — там же.
4. **Безопасность:** при **Aura** в общем случае ключ кэша должен включать tenant / user scope (**§20**); опционально уже **`context["tenant_id"]`** — см. реестр фич.
...
| **GraphCaster** | **План** полного цикла — **§23.2**. **Срез без ReAct:** **`ai_route`** (один запрос к провайдеру → ветка) — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) | Через **`task`**, скрипт или внешний HTTP; схема tool — в артефактах run | Минимум: без долговременной памяти в MVP | Расширение **`run-event`** для «tool_start/tool_end» при необходимости (**§3.7**) |

### 23.2. Планирование для GC

1. **ИИ на графе — не путать слои:** **Статическое** ветвление по ответу внешнего LLM (**нода `ai_route`**, один раунд, не цикл tool) — **закрыто** — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md). Полноценный **F11** (несколько раундов tool-calling, память) — по-прежнему **§23** ниже.
...
**F1** у конкурентов почти всегда **не** сводится к библиотеке «ноды+рёбра»: важны **хром ноды** (иконки, статусы, ошибки валидации), **модель ручек** (мульти-выход, типы шин — см. **§15**), **декорации полотна** (комментарии, группы), **навигация** (миникарта; поиск на полотне), **жесты** (мультивыбор, выравнивание) и **связь с сервером** (автосохранение vs локальный черновик). **Поиск на полотне**, **мультивыбор**, **буфер подграфа**, **групповое удаление**, защита **`start`** на canvas — факты в [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (разделы «Поиск…», «Мультивыбор…»); здесь в **§28** — планирование и сравнение конкурентов без повторения реализации. У **GraphCaster** осознанный выбор — **@xyflow/react** и единый путь **`ui/src/graph/*`** ↔ **`graph-document`** (**A**).
...
| **GraphCaster** | **@xyflow/react** | **`GcFlowNode`**, **`GcCommentNode`**, предупреждения структуры/ветвления; **`sanitizeGraphConnectivity`** при экспорте (**P2** — см. [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md)); инспектор; палитра «перейти к ноде»; мультивыбор, **Ctrl+C/V** подграфа, групповое удаление, защита **`start`** — тот же файл, раздел **«Мультивыбор…»** | **`GraphDocument`** + **`validate.py`**; без БД (**F23**) |
...
**Открытие документа (P1, ошибки файла / JSON / парсера):** **закрыто** — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md), раздел **«Открытие графа: ошибки JSON и парсера»** (`OpenGraphErrorModal`, `openGraphErrorPresentation.ts`, **`titleWithFile`**); детали и пути в коде — только там, **§28** не дублирует.
...
4. **Мини-карта / навигация:** виджет **MiniMap** из **@xyflow/react** уже в MVP полотна (см. **`DEVELOPMENT_PLAN.md`**, [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md)). **Поиск ноды на полотне** (палитра, фильтр по id/типу/лейблу и **`graph_ref`**) — **закрыто**: раздел «Поиск и переход к ноде на canvas» в [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md). **Открыто** на будущее: **виртуализация** или **lazy**-подграфы на больших графах только после профилирования (**§17** для потока событий — отдельно).
...
6. **Комментарии и группы:** **`GcCommentNode`** уже задаёт прецедент «неисполняемая нода»; группы (frame) — только если появятся в схеме **A** и фильтре **`validate.py`**.
7. **Co-edit (**F22**):** не синхронизировать сырой internal-state **React Flow** по сети — только **документ** (**§19**).
...
| **GraphCaster** | **`graphId`** → путь через **`workspace.py`** | Наследуется **`start`/`exit`** дочернего **A**-документа; рёбра родителя к **`graph_ref`** | Явный **лимит глубины** в раннере; **циклы** между файлами по workspace (**`graphId` → `targetGraphId`**) — **закрыто:** [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (**F5**); **§29.2** — ленивая загрузка, Aura, async и пр. |
...
4. **CI:** регрессионные JSON фикстуры **нескольких** `schemaVersion` в **`python/tests/`** или `examples/`, чтобы раннер не «забывал» старый формат до объявленного EOL. **Плюс** автоматический прогон в **GitHub Actions** при изменениях субмодуля: **`.github/workflows/graph-caster-ci.yml`** в корне **Aura** (**pytest** + **`npm test`/`build`**) — факты и ссылки — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) («CI в монорепозитории Aura»).
...
| **GraphCaster** | Ожидается **один входной** **`start`** и путь к **`exit`**; **порядок** исходящих рёбер из **A** задаёт перебор при ветвлении (**F4**). **Статический** список нод вне обхода от **`start`** (все рёбра считаются проходимыми) — см. [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (**F3**). Ноды **`fork`** (fan-out безусловных **`out_default`** в очередь), **`merge`** (**passthrough** и **barrier** join в одном процессе, последовательно) — там же, подраздел **Merge** в **F4**. | Вложение: синхронный **`graph_ref`** (**§29**) | Данные по связям документа; события **`node_execute`** и др. (**§3.7**) |
...
4. **Fork/join (несколько предков):** в **A** — **`merge`** (**passthrough** / **`barrier`**) и **`fork`**; исполнение веток **последовательно** в одном процессе — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (**F4** / Merge). **Не** закрыт паритет с **полным** **n8n Merge** при **OS-параллелизме** веток — **§13** / **F6**. До отдельного ТЗ не расширять семантику неявно (**PRODUCT_DESIGNE.md**).
...
**Закрыто в репозитории (факты, пути к коду, грамматика **`$json`** / **`$node`** / шаблонов):** только в разделе **«Условные рёбра / F4»** [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) — здесь не дублировать.
...
4. **ИИ-ветвление без полноценного F11:** нода **`ai_route`** (HTTP к провайдеру, **`choiceIndex`**, события **`ai_route_*`**) — **закрыто** — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md). **Остаётся открытым:** смешение «промпт в **`condition`**» и LLM без отдельной ноды; полный агентский цикл — **§23**.
...
| **GraphCaster** | **NDJSON** (stdout / файл) | Плоский **`type`** на корне объекта (**§3.7**) | Нет бинарных кадров в **`run-event`**; артефакты в **`runs/`** (**E**) | Dev **SSE** (`serve`): bounded очередь + **`stream_backpressure`** — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (**«Backpressure SSE»**). **Prod** WS/**pushRef**/relay — **§39.2** п.7, без дубля фактов |
...
6. **Backpressure (dev закрыт в субмодуле):** эталон проблемы — неограниченная очередь к сокету (**§13.3**, Comfy **`PromptServer.messages`**). **Dev run-брокер** (`serve`, SSE): bounded очереди, дроп **`process_output`**, **`stream_backpressure`**, параллельная доставка при нескольких подписчиках — факты и пути только в [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (**«Backpressure SSE»**); здесь не дублировать. Для **Aura** / **prod** и прочих мостов по-прежнему зафиксировать явную политику (глубина, дроп, relay); блокировать **`runner`/подпроцесс `task`** из-за медленного UI — только осознанно (**latence**).
...
**Точечные углубления позже:** сравнение с bounded queues в других стэках; лимиты прокси на длину SSE-чанка; политика **`QueueFull`** для **prod**-мостов **Aura** (dev `serve` — см. **«Backpressure SSE»** в [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md)); полный **v2 workflow** stream в backend Langflow vs **LFX** — при сближении с продуктовым Langflow HTTP.
