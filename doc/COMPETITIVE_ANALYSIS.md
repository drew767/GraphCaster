...
**Дата:** 2026-03-29 · **итерация 69** (**Follow run** / **`FollowActiveRunCamera`** — факты в [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md), строка **«Опционально следить камерой…»**; выполненный план удалён **`doc/PLAN_follow_active_node_during_run.md`**; папка **`doc/plans/`** по-прежнему только для временных черновиков)  
