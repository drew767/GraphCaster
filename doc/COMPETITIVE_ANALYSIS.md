...
| **GraphCaster** | **частично** | Слой **B** (**полотно, инспектор, предупреждения**, **@xyflow/react**, **`GcFlowNode`** / **`GcCommentNode`** / **`GcGroupNode`**): закрытые срезы и факты по **F1** / производительности canvas — только [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md), раздел **«Canvas: большие графы»** и смежные пункты (**MiniMap** в т.ч. **цвет по типу ноды**, **`minimapNodeColors.ts`**, поиск, ПКМ, мультивыбор, **snap-to-grid** / **align–distribute**, **LOD по zoom**, **F13**, **`group`** и т.д.). **Остаток F1 / §28.2:** **lazy**-подграфы в схеме **A**; опционально более лёгкий хром **полностью** вне вьюпорта (**§17**, профиль); полная типизация пинов — **§15**; **встраивание** дочернего графа в JSON родителя (**A**) вне file-first — **§29**. |
...
| **GraphCaster** | **@xyflow/react** | Редактор слоя **B** и сценарии **P1/P2** — единый реестр в [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (**«Canvas: большие графы»** в т.ч. **visible-only** + **LOD по zoom**, **MiniMap…**, **Поиск…**, меню добавления ноды, мультивыбор, **snap-to-grid / align–distribute**, **F13**, **`group`**, консоль, открытие графа и др.; **ленивое превью `graph_ref`** в инспекторе). **§28.2 п.4:** детали DOM/производительности не дублировать здесь; **остаток** — **lazy**-подграфы в **A**; опционально — хром полностью вне кадра (**§17**); **§15**; встраивание child JSON в **A** — **§29** | **`GraphDocument`** + **`validate.py`**; без БД (**F23**) |
