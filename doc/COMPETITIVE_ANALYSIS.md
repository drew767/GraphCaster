...
| 6 условия, ИИ-ветвление | B, D, F | F4, F11; **F19** in-graph — факты в [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md) (**§16** / **§37** для расширений) | **§32** (**F4**): семантика **`edge.condition`** и контекст; **§31** (**F3**) — только порядок списка рёбер; **Dify** **`SkipPropagator`** / условные ноды, **`FAIL_BRANCH`** (**§16**), **Flowise** condition nodes, **n8n** IF/Switch; полный **F11** — **§23** |
...
**Fork/join**, **ИИ-ветвление** и **расширения политики ошибок** (**F19**, **§16**, **§37**): merge/split — **n8n** и маршрутизация — **Dify**; отдельно от ретраев **`task`** в **`process_exec.py`**. In-graph **`out_error`** — [`IMPLEMENTED_FEATURES.md`](IMPLEMENTED_FEATURES.md); новые стратегии — согласовать в схеме и **`run-event`** (**§3.7**).
