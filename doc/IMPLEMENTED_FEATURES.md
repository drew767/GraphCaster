...
| Модалка ошибки открытия: копирование и busy-состояние | **`OpenGraphErrorModal`**: общий **`writeTextToClipboard`** (**`lib/clipboardWrite.ts`**); **`copyBusyRef`** выставляется **синхронно** с началом копирования (вместе с **`copyBusy`**), сброс в **`finally`**; **`safeClose`** проверяет **`copyBusyRef`**: пока идёт копирование, **не** вызывается **`onClose`** (**Escape**, backdrop, кнопка **Закрыть**); **`aria-busy`**, кнопки **Copy** и **Закрыть** **`disabled`** на время копирования; при **`open`** / смене **`presentation`** сбрасываются **`copyDone`**, **`copyBusy`** и ref |
