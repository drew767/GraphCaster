...
| Save / workspace: одна модалка, полевые ошибки без второй модалки поверх диалога | **`GraphSaveModal`**: под полем имени — **`SaveFieldIssue`** (пустое имя; **`getDocument()` === null** — **`document_unavailable`** / **`app.saveModal.documentUnavailable`**; сбой **`saveJsonWithFilePickerOrDownload`**; конфликт **`graphId`** с другим файлом в **`graphs/`**; сбой **`writeJsonFileToDir`**; потеря привязки папки — **`workspaceUnavailable`**); **`onSaveToWorkspace`** → **`Promise<GraphSaveToWorkspaceResult>`**; **`saveDocumentToWorkspace`** возвращает структурированный результат и **не** открывает **`AppMessageModal`** для duplicate **`graphId`** / ошибки записи на этом пути; **`role="alert"`**, **`aria-describedby`**, **`aria-busy`** на диалоге при сохранении; копирование текста ошибки через общий **`writeTextToClipboard`** (**`lib/clipboardWrite.ts`**) — те же ключи **`app.errors.openModal.copy`** / **`copied`**, что у **`OpenGraphErrorModal`**; **`isSaving`** — блокировка повторного Save, поля имени и плиток workspace; **`safeClose`** (**`isSavingRef`**) — пока идёт сохранение, **Escape**, клик по backdrop и **Отмена** **не** вызывают **`onClose`**, чтобы избежать гонки с **`setGraphDocument`** / **`rescanWorkspace`** после закрытия диалога (успешный Save по-прежнему закрывает через **`onClose`**) |

Код: **`ui/src/lib/clipboardWrite.ts`** (**`writeTextToClipboard`** — **`OpenGraphErrorModal.tsx`**, **`GraphSaveModal.tsx`**), **`ui/src/graph/openGraphErrorPresentation.ts`** (в т.ч. **`presentationForSave*`** / **`presentationForWorkspace*`** для Vitest и прочих вызовов; путь Save из **`GraphSaveModal`** их не использует), **`OpenGraphErrorModal.tsx`**, **`AppShell.tsx`**, **`InspectorPanel.tsx`**, **`GraphSaveModal.tsx`**; Vitest **`openGraphErrorPresentation.test.ts`**. В **`doc/DEVELOPMENT_PLAN.md`** пункт P1 по открытию — **закрыт**; в **`COMPETITIVE_ANALYSIS.md`** §**1** и §**28.2** — только отсылка сюда, **без** дублирования деталей реализации Save.
...
Срез **§13.3** / **§39.2** у конкурентов: **dev `serve` (SSE)** реализован здесь и в разделе **«Инкрементальный вывод подпроцесса `task`»** выше; **prod**-мост, **relay**, сессионный **`pushRef`** — в [`COMPETITIVE_ANALYSIS.md`](COMPETITIVE_ANALYSIS.md) **§39** / **§39.2** п.7 и сокращённый п.6 (политика **Aura** / хоста).
