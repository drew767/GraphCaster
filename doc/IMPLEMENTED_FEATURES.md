...
| Два уровня буфера (**§13.3** / **§39.2** у конкурентов) | **Ядро:** **`process_exec`** — очередь **`readline` → emit** с **`maxsize`** (этот раздел). **Транспорт (dev):** **`RunBroadcaster`** — раздел **«Backpressure SSE»** ниже. **Prod** / relay — по-прежнему в [`COMPETITIVE_ANALYSIS.md`](COMPETITIVE_ANALYSIS.md) **§39.2** п.7 |
