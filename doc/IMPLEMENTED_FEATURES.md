...
**SSOT по п.4:** закрытые факты (viewport **React Flow**, объём DOM, **LOD по zoom**, ленивое превью **`graph_ref`**, оверлей, инкрементальный sync) перечислены **только здесь**; в [`COMPETITIVE_ANALYSIS.md`](COMPETITIVE_ANALYSIS.md) §**28.2** п.**4** — отсылка сюда без дублирования перечня. Исторический план в **`doc/plans/`** по **LOD** снят после внедрения. Открытые хвосты конкурентного сравнения — опциональный хром **полностью** вне кадра (**§17**), **§15**, **§29**.
...
| **Ленивое превью вложенного графа** по **`graph_ref`** (как n8n / Dify: полное содержимое дочернего файла не нужно до явного жеста) | **`InspectorPanel`**: сворачиваемая секция «по запросу» + **`Refresh`**; **`getGraphRefWorkspaceHint`**: если цели нет в индексе **`graphs/`** — одно сообщение «нет в индексе», без чтения диска, Expand/Refresh отключены; после **`await loadGraphRefSnapshot`** обновление state только если не сменилось выделение (ref-поколение / ключ инспектора); **`AppShell`**: **`loadGraphRefSnapshot`**, in-memory кэш по **`targetGraphId`**, сброс при **`rescanWorkspace`** и точечная инвалидация ключа по **`graphIdFromDocument`** после успешного autosave; **`graphRefLazySnapshot.ts`**, Vitest **`graphRefLazySnapshot.test.ts`**; i18n **`app.inspector.graphRefPreview*`** |

Код: **`ui/src/components/GraphCanvas.tsx`**, **`ui/src/components/GcCanvasLodContext.tsx`**, **`ui/src/graph/canvasLod.ts`**, Vitest **`canvasLod.test.ts`**, **`ui/src/graph/gcFlowEdgeSync.ts`**, Vitest **`gcFlowEdgeSync.test.ts`**; **`ui/src/run/runSessionStore.ts`** (**`nodeRunOverlayRevision`**), **`ui/src/run/nodeRunOverlay.ts`**, Vitest **`nodeRunOverlayBatch.test.ts`**. Локальный стресс-фикстур (stdout, не коммитить большие JSON): **`npm run fixture:large-graph`** в **`ui/`** (аргумент — число нод, по умолчанию **500**). **Baseline / Chrome Performance / целевые пороги** (процедура и таблица критериев): **`ui/README.md`** раздел **«Большой граф: фикстура и baseline»**.
...
Сравнение с конкурентами и **остаток** F1 (опционально дополнительный «лёгкий» хром **полностью** за пределами вьюпорта по профилю — сверх **`onlyRenderVisibleElements`** и **LOD по zoom**; **встраивание** дочернего JSON в слой **A** вне file-first — **§29**) — [`COMPETITIVE_ANALYSIS.md`](COMPETITIVE_ANALYSIS.md) **§28**, таблица **F1** (строка GraphCaster).
