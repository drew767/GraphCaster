...
| n8n: список шагов execution (node before/after) параллельно «сырой» ленте | Вкладка **Steps** в **`ConsolePanel`**: редьюсер **`reduceConsoleLinesToRunTimeline`** в **`ui/src/run/buildRunTimeline.ts`** из того же буфера NDJSON — статус строки (**running / success / failed / skipped / cancelled / partial**; **`cancelled`**/**`partial`**/`failed` с **`run_finished`** из **`runner.py`**, неизвестный **`status`** → **failed**), тип ноды, длительность при parseable **`ts`/`at`/`timestamp`** на событиях, агрегация **`agent_step`** в **`summary`**; клик → **`onNavigateToNode`** как у строки лога |
