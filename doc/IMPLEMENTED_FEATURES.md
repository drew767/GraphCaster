...
| Модалка ошибки открытия: копирование и busy-состояние | **`OpenGraphErrorModal`**: общий **`writeTextToClipboard`** (**`lib/clipboardWrite.ts`**); при копировании — **`copyBusy`**, **`aria-busy`** на **`role="dialog"`**, кнопка **Copy** **`disabled`** до завершения **`writeTextToClipboard`**; при **`open`** / смене **`presentation`** сбрасываются **`copyDone`** и **`copyBusy`** |
