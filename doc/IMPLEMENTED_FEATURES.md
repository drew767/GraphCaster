...
| Отмена прогона из MCP-процесса | **`graphcaster_cancel_run`** (`run_id` — UUID прогона) → **`get_default_run_registry().request_cancel`** (та же кооперативная отмена, что при **`timeout_sec`** у **`graphcaster_run_graph`**). **Интеграторам:** **`ok: true`** здесь только «валидный запрос» (после trim); факт постановки отмены — **`cancelRequested`**; при **`false`** — **`reason`**: **`unknown_run_id`** \| **`run_not_active`**. Отмена **другого** процесса (брокер / **`--track-session`**) по-прежнему через соответствующий транспорт |
| Исполнение | Тот же **`GraphRunner`** + **`RunHostContext`**, события в in-memory sink (**без** NDJSON на stdout — stdio занят протоколом MCP) |

**Сопоставление с конкурентами (отмена прогона по стабильному id, кооперативно):** у **n8n** — останов по **`executionId`**; у **Dify** — команды движку (**`CommandChannel`**, abort); у **Langflow** / **Flowise** — прерывание сессии исполнения на стороне сервера; **GraphCaster** в MCP — **`graphcaster_cancel_run`** → **`RunSessionRegistry.request_cancel`** для **`run_id`** из ответа **`graphcaster_run_graph`** (тот же механизм, что при **`timeout_sec`**). Подробная сводная таблица продуктов по MCP — [`COMPETITIVE_ANALYSIS.md`](COMPETITIVE_ANALYSIS.md) **§34**.
...
| Автодополнение выражений в условии ребра и URL **`http_request`** | Подсказки для **`$json`**, **`$node`**, **`$env`**, **`$node["…"]` / `['…']`** по id нод текущего графа, имён **builtins** выражения (как **`python/graph_caster/expression/functions.py`**); **Ctrl+Space** — полный список (native input); опционально **Monaco** (**`editor="monaco"`**, **`ExpressionMonacoField.tsx`**, язык **`graphcaster-expression`**, чекбокс + **`gc.inspector.expressionMonaco`**); **`scanExpressionTemplateSyntax`** для **`{{ … }}`** → **`aria-invalid`** / **`gc-expression-ac--invalid`**; **`expressionAutocomplete.ts`**, **`ExpressionAutocompleteInput.tsx`**, **`InspectorPanel.tsx`**, **`vite-plugin-monaco-editor`**, стили **`app.css`**, i18n; Vitest **`expressionAutocomplete.test.ts`** |
...
| Метрики брокера и лимиты fork | **`GET /metrics`**: **`gc_graph_fork_threadpool_max_config`** (**`GC_GRAPH_FORK_THREADPOOL_MAX`**, иначе **`GC_RUNNER_MAX_WORKERS`**, **0** если не задано); **`WorkerPool.in_flight_count`** — незавершённые futures |
...
| Идея конкурента | Реализация GC |
|-----------------|---------------|
| Индекс документов и retrieval (Dify / Langflow / Flowise) | Ноды **`rag_index`** / **`rag_query`**: **`rag_index_exec`**, **`rag_query_exec`**; чанкинг **`rag/text_split.py`**; эмбеддинги **`rag/embed_dispatch.rag_embed_chunk`** — по умолчанию **`hash_embedding`**, опц. **`GC_RAG_EMBEDDING_BACKEND=openai`** / **`sentence_transformers`** (**`[rag-embed-local]`**) |
...
| Тесты | **`python/tests/test_rag_index_node.py`**, **`test_rag_query_exec.py`**, **`test_rag_vector_backends.py`**, **`test_rag_embed_dispatch.py`** (часть кейсов пропускается без optional deps) |
...
| n8n: список шагов execution (node before/after) параллельно «сырой» ленте | Вкладка **Steps** в **`ConsolePanel`** → компонент **`ExecutionTimeline.tsx`**: редьюсер **`reduceConsoleLinesToRunTimeline`** в **`ui/src/run/buildRunTimeline.ts`** из того же буфера NDJSON — статус строки (**running / success / failed / skipped / cancelled / partial**; **`cancelled`**/**`partial`**/`failed` с **`run_finished`** из **`runner.py`**, неизвестный **`status`** → **failed**), тип ноды, длительность при parseable **`ts`/`at`/`timestamp`**, агрегация **`agent_step`** в **`summary`**; **`maxTimelineDurationMs`**, **`assignTimelineLanes`**, полоса длительности; i18n **`app.console.timelineStatus.*`**; клик / клавиатура → **`onNavigateToNode`** как у строки лога |
...
| Dify-style OTLP / внешний backend трасс | Опционально **`pip install -e ".[otel]"`** и env (**`GC_OTEL_EXPORTER_OTLP_ENDPOINT`** или **`OTEL_EXPORTER_OTLP_*`**): корневой span **`gc.run`**, дочерние **`gc.node`** на визит ноды (в т.ч. ветки **`fork`** в worker-потоках); атрибуты **`graph_caster.*`**; логические ошибки визита помечают span статусом **ERROR** (`otel_tracing.mark_current_span_error`). Контракт **`run-event`** / NDJSON **без** полей trace (по желанию позже: **`traceparent`** в subprocess, **`traceId`** в событиях). См. **`python/README.md`**. |
| Тесты | `ui/src/run/consoleLineMeta.test.ts`, `ui/src/run/buildRunTimeline.test.ts`, `ui/src/run/nodeRunOverlay.test.ts`, `ui/src/run/parseRunEventLine.test.ts` (**`peekRootGraphIdFromNdjson`**) (Vitest); Python **`tests/test_otel_tracing.py`** (extra **`[otel]`**) |

Код: `ui/src/run/consoleLineMeta.ts`, `ui/src/run/buildRunTimeline.ts`, `ui/src/run/nodeRunOverlay.ts`, `ui/src/run/runEventSideEffects.ts`, `ui/src/run/runSessionStore.ts`, `ui/src/run/parseRunEventLine.ts`, `ui/src/components/ConsolePanel.tsx`, `ui/src/components/GraphCanvas.tsx`, `ui/src/components/nodes/GcFlowNode.tsx`, `ui/src/layout/AppShell.tsx`, стили консоли и оверлея в `ui/src/styles/app.css`, i18n `app.console.*`, `app.run.overlay.*`; OpenTelemetry — `python/graph_caster/otel_tracing.py`, `python/graph_caster/runner.py`, `python/README.md`.
...
**Отложено (отдельный тикет / процесс, не блокер итерации):** (1) коммит набора изменений в репозитории **GraphCaster** и обновление указателя субмодуля **`third_party/graph-caster`** в монорепо-носителе; (2) опциональная полная **focus trap** для модалки History (WCAG) поверх Escape + стрелок на вкладках; (3) теоретический край точности JSON **`rebuilt`** как числа в веб-клиенте при счётчике **> 2⁵³** — при необходимости брокер может отдавать строку, как уже делает Tauri-мост.

**Заметки по контракту:** клиент UI шлёт **`limit`** по умолчанию **500**; тело без **`limit`** в **`POST /run-catalog/list`** даёт серверный дефолт **100** (`run_broker/app.py`) — для сторонних HTTP-клиентов держать это в уме; Tauri **`gc_list_run_catalog`** при **`limit ≤ 0`** подставляет **500** (как serde-default запроса).
