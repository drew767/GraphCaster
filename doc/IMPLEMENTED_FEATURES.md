...
Статус в competitive: закрытые пункты **§32.2** по **`branch_*`** и **`edge_traverse`** сведены в таблицу ниже; ноды **`fork`**, **`merge`** (**passthrough** / **`barrier`**) — отдельный подраздел после таблицы. В **`COMPETITIVE_ANALYSIS.md`** (**§32.2**, «Открыто») остаются полноценный **n8n Expression**, ИИ-ветвление (**фаза 6**), продуктовая документация, расширение контекста предикатов и **истинный параллелизм веток** (**F6**); однопоточный **fan-out/join** закрыт здесь, а не в списке «открыто» competitive. In-graph **`out_error`** (**F19**) закрыт здесь и отражён в **§16** / **§37** competitive без дублирования объёма реализации.
...
Остаток до полного паритета F17 / n8n (транзитивное замыкание **dirty**, кэш не-**`task`**, TTL и т.д.) — в [COMPETITIVE_ANALYSIS.md](COMPETITIVE_ANALYSIS.md) (раздел **22.2**).
...
| **Десктоп (Tauri):** явный **`--step-cache`** / **`--step-cache-dirty`** как у n8n **`dirtyNodeNames`** | Панель **Run**: чекбокс «Step cache» (требует непустой корень воркспейса = **`--artifacts-base`**); бейдж — число id в очереди **dirty**. Очередь **dirty** очищается после **успешного** старта run (нет исключения **`invoke`**); **`--step-cache`** в Rust не добавляется без непустого **`artifacts_base`**. Пустой путь артефактов сбрасывает чекбокс Step cache. **`InspectorPanel`** для **`task`**: **`data.stepCache`**, кнопка «Mark dirty for next run». **`run_bridge.rs`** → **`gcStartRun`** (`run/runCommands.ts`). Состояние очереди: **`run/stepCacheDirtyStore.ts`**; на ноде — бейдж **`C`** в **`GcFlowNode`**. Локальное сохранение намерения кэша: **`localStorage`** **`gc.run.stepCacheEnabled`** |

Код: `python/graph_caster/document_revision.py`, `node_output_cache.py`, `runner.py`, `__main__.py`; `ui/src-tauri/src/run_bridge.rs`, `ui/src/run/runCommands.ts`, `ui/src/run/stepCacheDirtyStore.ts`, `ui/src/layout/AppShell.tsx`, `ui/src/components/TopBar.tsx`, `ui/src/components/InspectorPanel.tsx`, `ui/src/components/nodes/GcFlowNode.tsx`. Тесты: `test_document_revision.py`, `test_node_output_cache.py`, `test_runner_step_cache.py`, `test_run_event_schema.py`, `test_cli_main.py`. Документация CLI: `python/README.md`.
