...
Статус в competitive: факты реализации **F4** (в т.ч. **`$json`**, **`branch_*`**, **`edge_traverse`**) — в **§32.1–§32.2** [`COMPETITIVE_ANALYSIS.md`](COMPETITIVE_ANALYSIS.md) со ссылкой сюда; ноды **`fork`**, **`merge`** (**passthrough** / **`barrier`**) — отдельный подраздел ниже. В **§32.2** список «**Открыто**» — полный **n8n Expression** runtime, ИИ-ветвление (**фаза 6**), продуктовая документация, расширение контекста предикатов, **истинный параллелизм** веток (**F6**); узкий конверт **`$json`** без VM — **закрыт** (таблица ниже). In-graph **`out_error`** (**F19**) закрыт здесь и отражён в **§16** / **§37** competitive без дублирования объёма реализации.
...
| Выражения к данным шага без небезопасного eval | Подмножество **JSON Logic** в строке **`edge.condition`** (JSON-объект с одной корневой операцией); **шаблоны** **`{{dotted.path}}`** (truthiness) и **`{{dotted.path}} <op> <literal>`** (`op` ∈ `==`, `!=`, `<`, `<=`, `>`, `>=`; кавычки для строковых литералов; для **`==`/`!=`** — числовое приведение строки и числа как **`_coerce_num`**; литерал сравнения **без** многострочного «растягивания» regex); корень пути может быть зарезервированный **`$json`** (снимок **`last_result`**, см. строку «Контекст предиката»); иначе legacy-литеры **`true`**/**`false`**/…; иначе не-JSON-строка без `{{` → **`bool(context["last_result"])`**. Реализация: **`graph_caster/edge_conditions.py`**, статический разбор в UI: **`edgeConditionTemplates.ts`** / предупреждения в **`branchWarnings.ts`** и **`AppShell`** (в т.ч. **`too_long`** при превышении **`MAX_EDGE_CONDITION_CHARS`**, см. `python/README.md`) |
| Контекст предиката | **`last_result`**, **`node_outputs`**, пути **`var`** через **`.`** (напр. **`node_outputs.t1.processResult.exitCode`**); **синтетический корень** **`$json`** = **`last_result`** если **`dict`**, иначе **`{"value": last_result}`** (перезаписывает одноимённый ключ во **`context`** при оценке). Скрыты только **корневые** ключи **`context`** с префиксом **`_`**; вложенные поля под **`node_outputs`** не маскируются |
...
**Открыто в F4 (см. `COMPETITIVE_ANALYSIS.md` §32.2):** полноценный **n8n Expression** (произвольные функции, **`$node[…]`** как у n8n, sandbox VM) — **вне** безопасной грамматики JSON Logic + mustache + узкого конверта **`$json`** (таблица выше). **Fan-out/join** в одном процессе (**`fork`**, **`merge`** **`barrier`**) — в таблице **Merge** ниже. Остаётся в competitive: **истинный параллелизм веток** (worker pool / **F6**), ИИ-ветвление без отдельной ноды (**фаза 6**). In-graph **`out_error`** — раздел **F19** ниже.
...
**Снято с «остатка» [COMPETITIVE_ANALYSIS.md](COMPETITIVE_ANALYSIS.md) §22.2 (ранее «транзитивное `dirtyNodeNames`»):** очередь **`--step-cache-dirty`** в десктопе пополняется транзитивно по DAG **успешных** рёбер (как предки в ключе F17 в **`runner.py`**); правки **`condition`** / новое ребро (seed **`source`**); типы нод с **`onApplyNodeData`** — см. строку таблицы про **`dirtyNodeNames`** ниже. **Пока вне среза:** инвалидация при правках **внутри** вложенного **`graph_ref`**; кэш не-**`task`**, TTL, тоггл step-cache на полотне — см. **22.2** в [COMPETITIVE_ANALYSIS.md](COMPETITIVE_ANALYSIS.md).
...
| n8n: **`dirtyNodeNames`** / транзитивная инвалидация при partial | **Десктоп:** по успешным рёбрам (не **`out_error`**), как предки в ключе F17 в **`runner.py`**; в очередь попадают только **`task`** с **`data.stepCache`** truthy; кнопка «Mark dirty» берёт граф с канваса (**`exportDocument`**); **`onApplyNodeData`** вызывает замыкание только для исполняемых типов (не **`comment`**); смена **`condition`** ребра и новое ребро — seed **`source`**; лог консоли: дельта **`+N`** и полная очередь |
...
Код: `python/graph_caster/document_revision.py`, `node_output_cache.py`, `runner.py`, `__main__.py`; `ui/src-tauri/src/run_bridge.rs`, `ui/src/run/runCommands.ts`, `ui/src/run/stepCacheDirtyStore.ts`, `ui/src/graph/stepCacheDirtyGraph.ts`, `ui/src/layout/AppShell.tsx`, `ui/src/components/TopBar.tsx`, `ui/src/components/InspectorPanel.tsx`, `ui/src/components/nodes/GcFlowNode.tsx`. Тесты: `test_document_revision.py`, `test_node_output_cache.py`, `test_runner_step_cache.py`, `test_run_event_schema.py`, `test_cli_main.py`; Vitest **`stepCacheDirtyGraph.test.ts`**, **`stepCacheDirtyStore.test.ts`**. Документация CLI: `python/README.md`.
