...
| **Backpressure SSE (dev-брокер)** | У каждого подписчика **`RunBroadcaster`** — **`queue.Queue(maxsize=…)`** (**`GC_RUN_BROKER_SUB_QUEUE_MAX`**, по умолчанию **8192**, clamp **64…131072**; в **`RunBroadcasterConfig`** значения **`max_sub_queue_depth` ≤ 0** нормализуются до **1**). События **`process_output`** в stdout при переполнении **отбрасываются** (новый кадр); **`run_finished`**, **`run_started`**, прочий NDJSON и **`err`/`exit`** — **не** отбрасываются (блокирующий **`put`** даёт backpressure на чтение stdout подпроцесса). В поток попадает **`stream_backpressure`** (схема **`schemas/run-event.schema.json`**, throttle **~100 ms** на подписчика). Консоль UI: **`app.run.console.outputTruncated`**, поиск по строке `stream_backpressure dropped`. **Сопоставление с competitive doc:** п.6 **§39.2** `COMPETITIVE_ANALYSIS.md` (общая норма backpressure) — здесь зафиксирована **фактическая** реализация для dev **SSE**; **prod** `pushRef` / relay — по-прежнему в **§39**. Код: `python/graph_caster/run_broker/broadcaster.py`, `registry.py`; тесты: **`python/tests/test_run_broker_backpressure.py`**, `test_run_event_schema.py` |
