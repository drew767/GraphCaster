...
**SSOT по п.4:** закрытые факты (viewport **React Flow**, объём DOM, **LOD по zoom**, ленивое превью **`graph_ref`** в инспекторе — без встраивания дочернего JSON в документ родителя, оверлей, инкрементальный sync) перечислены **только здесь**; в [`COMPETITIVE_ANALYSIS.md`](COMPETITIVE_ANALYSIS.md) §**28.2** п.**4** — отсылка сюда без дублирования перечня. Выполненные черновики в **`doc/plans/`** удаляются после внедрения (в т.ч. по **LOD** и по **ленивому превью `graph_ref`**). Открытые хвосты конкурентного сравнения — опциональный хром **полностью** вне кадра (**§17**), **§15**, **§29** (встраивание дочернего графа в **A**).
