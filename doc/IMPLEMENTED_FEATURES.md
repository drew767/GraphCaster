...
Закрывает **§28.2** п.8 в [`COMPETITIVE_ANALYSIS.md`](COMPETITIVE_ANALYSIS.md) (таблица **F1**, строка **GraphCaster** — только отсылка сюда, без дублирования перечня): подстрока **«Подписи на рёбрах…»** в таблице выше.

Код: **`ui/src/components/GraphCanvas.tsx`**, **`ui/src/components/edges/GcBranchEdge.tsx`**, **`ui/src/components/edges/GcBranchEdgeUiContext.tsx`**, **`ui/src/graph/edgeCanvasLabel.ts`**, **`ui/src/graph/canvasEdgeLabels.ts`**, **`ui/src/components/GcCanvasLodContext.tsx`**, **`ui/src/components/GcViewportTierContext.tsx`**, **`ui/src/graph/canvasLod.ts`**, **`ui/src/graph/viewportNodeTier.ts`** (**`EMPTY_NODE_VISIBILITY_BY_ID`**), **`ui/src/graph/useGcEffectiveNodeTier.ts`**, **`ui/src/graph/canvasGhostOffViewport.ts`**, Vitest **`canvasLod.test.ts`**, **`viewportNodeTier.test.ts`**, **`gcEffectiveNodeTier.test.ts`**, **`edgeCanvasLabel.test.ts`**, **`canvasEdgeLabels.test.ts`**, **`fromReactFlow.test.ts`** (тип **`gcBranch`**), **`ui/src/graph/gcFlowEdgeSync.ts`**, Vitest **`gcFlowEdgeSync.test.ts`**; **`ui/src/run/runSessionStore.ts`** (**`nodeRunOverlayRevision`**), **`ui/src/run/nodeRunOverlay.ts`**, Vitest **`nodeRunOverlayBatch.test.ts`**. Локальный стресс-фикстур (stdout, не коммитить большие JSON): **`npm run fixture:large-graph`** в **`ui/`** (аргумент — число нод, по умолчанию **500**). **Baseline / Chrome Performance / целевые пороги** (процедура и таблица критериев): **`ui/README.md`** раздел **«Большой граф: фикстура и baseline»**.
...
| Статические **типы данных на портах** (фаза 1 F18, ориентир **Langflow** `validate_edge` / **Dify** declarative node typing) | Закрытый перечень **`PortDataKind`**: **`any`** \| **`json`** \| **`primitive`**; таблица по **`node.type`** + id ручки — **`ui/src/graph/portDataKinds.ts`**, **`ui/src/graph/portDataKindCompat.ts`** (матрица: **`any`** сочетается со всем; **`json`↔`primitive`** — предупреждение, без **`GraphStructureError`**); те же правила в Python — **`python/graph_caster/port_data_kinds.py`**, событие **`structure_warning`** при run через **`find_port_data_kind_warnings`** в **`runner.py`**. Валидация имени ручки по-прежнему в **`handle_contract`** / **`find_handle_compatibility_violations`** (жёсткая ошибка). Тесты: **`ui/src/graph/portDataKinds.test.ts`**, **`portDataKindCompat.test.ts`**, расширенный **`handleCompatibility.test.ts`**, **`python/tests/test_port_data_kinds.py`**. |
...
**Остаток типизации портов (планирование в [`COMPETITIVE_ANALYSIS.md`](COMPETITIVE_ANALYSIS.md) §15.2 — без дублирования полного списка здесь и там):** опциональные поля в **`graph-document`** / **`edges[].data`** для переопределения **`PortDataKind`**; вид **`blob_ref`** и другие виды сверх **`any`/`json`/`primitive`**; мультишины рёбер в духе **n8n**; доменные типы **Comfy** (IMAGE/LATENT и т.п.) — только при явном продуктовом домене медиа. Новый **`kind`** ноды задаёт набор портов — **§18** (**F15**) в том же competitive.
...
**Не сделано** (см. [`COMPETITIVE_ANALYSIS.md`](COMPETITIVE_ANALYSIS.md) **§21**; **co-edit / F22** — **§28.2** п.8): отдельная история **viewport**; отдельные **команды** с `apply`/`revert` per op (как в **Dify**); конфликт «файл на диске изменён снаружи» при autosave; **Yjs** для undo при **F22**.
