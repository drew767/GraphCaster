...
Список **моделей** в выпадающем списке заполняется из Cursor CLI (`agent models` для выбранного исполняемого файла; при отсутствии CLI — запасной вариант `composer-2`). Редактировать id модели вручную нельзя.

Поле **«Количество циклов к выполнению»** (`-Cycles`) в pipeline задаёт число циклов; в **линейном** режиме (`-Sequential`) значение **N>1** задаёт число раундов (в скрипте это сводится к прежней паре `-Loop` / `-MaxRounds`). Пауза между шагами (`-DelaySeconds`) в окне не настраивается — только через CLI при ручном запуске `agent-queue.ps1`.
