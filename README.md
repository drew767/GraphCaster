...
- **MCP (stdio):** опционально **`pip install -e ".[mcp]"`**, затем **`python -m graph_caster mcp -g ./graphs`** — tools для списка и запуска графов из внешнего MCP-клиента (см. `python/README.md`, `doc/IMPLEMENTED_FEATURES.md`).
...
| 7 | Run: **веб** — локальный SSE-брокер (`python -m graph_caster serve`) + прокси Vite; десктоп — Tauri + NDJSON (см. `ui/README.md`). |
