...
const NODE_BRACKET = `(?:\\$node\\[\\s*"[^"]*"\\s*\\]|\\$node\\[\\s*'[^']*'\\s*\\])(?:\\.${PATH_SEGMENT})*`;
const DOTTED_PATH = `(?:\\$json(?:\\.${PATH_SEGMENT})*|${NODE_BRACKET}|\\$node\\.${PATH_SEGMENT}(?:\\.${PATH_SEGMENT})*|\\$node(?![.\\[])|${PATH_SEGMENT}(?:\\.${PATH_SEGMENT})*)`;
